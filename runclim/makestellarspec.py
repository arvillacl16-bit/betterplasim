...
def _coarsen(wvls, fluxes, w1, w2, num=5000):
    ...
    for n in range(0, num):
        ...
        for v in range(0, inc):
            intg += (
                0.5 * (wvls[v1 + v] - wvls[v0 + v]) * (fluxes[v1 + v] + fluxes[v0 + v])
            )
...
def _processArgs():
    ...
    parser = ag.ArgumentParser(
        description="""Convert spectral files to exoplasim-compliant formats, including resampling to the necessary resolutions.
    ...
    parser.add_argument(
        ...
        "--plot",
        action="store_true",
        help="Display plots of the converted spectrum",
    )
    parser.add_argument(
        "-w",
        "--numwavelengths",
        default=2048,
        type=int,
        help="Number of wavelengths in the hi-res spectrum",
    )
    parser.add_argument(
        "-n", "--normalize", action="store_true", help="Normalize the output spectra"
    )
...
def convert(spectrumfile, name, plot=False, numwavelengths=2048, normalize=False):
    ...
    w2 = np.concatenate(
        [
            np.geomspace(0.2, 0.75, num=int(numw / 2) + 1)[:-1],
            np.geomspace(0.75, 100.0, num=int(numw / 2)),
        ]
    )
    if plot:
        plt.plot(w2)
