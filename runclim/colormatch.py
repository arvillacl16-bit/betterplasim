...
cie_wvl, cie_xx, cie_yy, cie_zz = np.loadtxt(
    "/".join(__file__.split("/")[:-1]) + "/cmf.csv", unpack=True, delimiter=","
)


def _loadcie():
    wvl, xx, yy, zz = np.loadtxt(
        "/".join(__file__.split("/")[:-1]) + "/cmf.csv", unpack=True, delimiter=","
    )
    return wvl, xx, yy, zz
...
def makexyz(wvl, spec, interpolant=None):
    ...
    XI = np.trapz(
        xn[~np.isnan(specn)] * specn[~np.isnan(specn)], x=wn[~np.isnan(specn)]
    )
    YI = np.trapz(
        yn[~np.isnan(specn)] * specn[~np.isnan(specn)], x=wn[~np.isnan(specn)]
    )
    ZI = np.trapz(
        zn[~np.isnan(specn)] * specn[~np.isnan(specn)], x=wn[~np.isnan(specn)]
    )
...
illuminantsxy = {
    "A": np.array(
        [
            illuminants["A"][0] / np.sum(illuminants["A"]),
            illuminants["A"][1] / np.sum(illuminants["A"]),
        ]
    ),
    "B": np.array(
        [
            illuminants["B"][0] / np.sum(illuminants["B"]),
            illuminants["B"][1] / np.sum(illuminants["B"]),
        ]
    ),
    "C": np.array(
        [
            illuminants["C"][0] / np.sum(illuminants["C"]),
            illuminants["C"][1] / np.sum(illuminants["C"]),
        ]
    ),
    "D50": np.array(
        [
            illuminants["D50"][0] / np.sum(illuminants["D50"]),
            illuminants["D50"][1] / np.sum(illuminants["D50"]),
        ]
    ),
    "D55": np.array(
        [
            illuminants["D55"][0] / np.sum(illuminants["D55"]),
            illuminants["D55"][1] / np.sum(illuminants["D55"]),
        ]
    ),
    "D65": np.array(
        [
            illuminants["D65"][0] / np.sum(illuminants["D65"]),
            illuminants["D65"][1] / np.sum(illuminants["D65"]),
        ]
    ),
    "D75": np.array(
        [
            illuminants["D75"][0] / np.sum(illuminants["D75"]),
            illuminants["D75"][1] / np.sum(illuminants["D75"]),
        ]
    ),
    "E": np.array(
        [
            illuminants["E"][0] / np.sum(illuminants["E"]),
            illuminants["E"][1] / np.sum(illuminants["E"]),
        ]
    ),
    "F2": np.array(
        [
            illuminants["F2"][0] / np.sum(illuminants["F2"]),
            illuminants["F2"][1] / np.sum(illuminants["F2"]),
        ]
    ),
    "F7": np.array(
        [
            illuminants["F7"][0] / np.sum(illuminants["F7"]),
            illuminants["F7"][1] / np.sum(illuminants["F7"]),
        ]
    ),
    "F11": np.array(
        [
            illuminants["F11"][0] / np.sum(illuminants["F11"]),
...
for gamut in _gamuts:
    if "_" in gamut:
        ...
        il = "D50"
    white = np.array(
        xyz2rgb(illuminantsxy[il][0], illuminantsxy[il][1], 1.0, gamut=gamut)
    )
    extranorm = (
        1.0 / white.max()
    )  # So the equal-power colour has a max RGB value of 1.0
...
def specs2rgb(wvl, specs, gamma=True, gamut="sRGB"):
    ...
    for n in range(0, len(specs)):
        colors[n, :] = xyz2rgb(
            intensities[n, 0], intensities[n, 1], norms[n], gamut=gamut
        )
    if gamma is True:
        colors[colors < 0.0031308] = 12.92 * colors[colors < 0.0031308]
        colors[colors >= 0.0031308] = (
