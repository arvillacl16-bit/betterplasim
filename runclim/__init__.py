...
import glob
import runclim.gcmt
import runclim.pyburn
import runclim.filesupport
from runclim.filesupport import SUPPORTED
import runclim.randomcontinents
import runclim.makestellarspec
import runclim.surfacespecs
import runclim.constants
from runclim.constants import *
...
def compile_pyfft():
    ...
    sourcedir = "/".join(
        __file__.split("/")[:-1]
    )  # Get the absolute path for the module
    print(sourcedir)
    ...
    try:
        ...
        if (
            not os.path.isfile(sourcedir + "/most_compiler")
...
def printsysconfig(ncpus=4):
    ...
    if ncpus == 1:
        if not os.path.exists(sourcedir + "/most_compiler"):
            raise Exception(
                """ExoPlaSim has not yet been configured. Please configure it by
...
class Model(object):
    ...
    def __init__(
        ...
        self.extensions = {
            "regular": self.extension,
            "snapshot": self.extension,
            "highcadence": self.extension,
        }
        self._configuredpostprocessor = {
            "regular": False,
            "snapshot": False,
            "highcadence": False,
        }

        self.postprocessordefaults = {
            "regular": {},
        ...
        if self.extension not in pyburn.SUPPORTED:
            raise Exception(
                "Unsupported output format detected. Supported formats are:\n\t\n\t%s"
        ...
        if (
            not os.path.isfile(sourcedir + "/firstrun")
            or not os.path.isfile(sourcedir + "/most_compiler")
            or not os.path.isfile(sourcedir + "/most_compiler_mpi")
        ):  # This means we haven't run yet, and have some post-install work to do
            recompile = True
        ...
        ):  # There is no need to set these for precompiled binaries
            recompile = True
        ...
        if os.path.isfile(
            self.workdir
        ):  # Linux can't have file and directory of same name
            self.workdir += "_dir"
        ...
        if resolution == T21:
            ...
            raise ValueError(
                "Resolution unsupported. ExoPlaSim supports T21, T42, T63, T85, "
        ...
        self.executable = self.executable.split("/")[
            -1
        ]  # Strip off all the preceding path
    ...
    def run(self, **kwargs):
        ...
        if not self.runscript:
            ...
            try:
                self.runscript(
                    self, **kwargs
                )  # runscript MUST accept a Model object as the first arg
            except Exception as e:
                print(e)
                self._crash()
    ...
    def runtobalance(
        ...
        os.system(
            "rm %s/runtimes.log" % self.workdir
        )  # We only want runtimes for this run
        ...
        while (
            not self._isbalanced(threshold=self.threshold, baseline=baseline)
            or self.currentyear < minyears
        ) and self.currentyear < runlimit:
            ...
            try:
                if float(sys.version[:3]) >= 3.5 and float(sys.version[:3]) < 3.7:
                    subprocess.run(
                        [self._exec + self.executable], shell=True, check=True
                    )
                elif float(sys.version[:3]) >= 3.7:
                    subprocess.run(
                        [self._exec + self.executable],
                        shell=True,
                        check=True,
                        capture_output=True,
                    )
                ...
                os.system(
                    "[ -e hurricane_indicators ] && mv hurricane_indicators "
                    + stormname
                )
                print(
                    "[ -e hurricane_indicators ] && mv hurricane_indicators "
                ...
                highcdn = 0
                try:
                    timeavg = self.postprocess(
                        dataname, None, log="burnout", crashifbroken=crashifbroken
                    )
                    if self.snapshots:
                        snapsht = self.postprocess(
                            snapname,
                            None,
                            ftype="snapshot",
                            log="snapout",
                            crashifbroken=crashifbroken,
                        )
                        os.system("mv %s%s snapshots/" % (snapname, self.extension))
                    if self.highcadence["toggle"]:
                        highcdn = self.postprocess(
                            hcname,
                            None,
                            ftype="highcadence",
                            log="hcout",
                ...
                if diagnosticvars is not None:
                    ...
                    for dv in diagnosticvars:
                        try:
                            print(
                                "\t%9s:\t%f"
                                % (dv, self.inspect(dv, year=-1, savg=True, tavg=True))
                            )
                        except:
                            print(
                                "\tError computing global annual mean for variable %s"
                                % dv
                            )
                if clean:
                    if timeavg:
                ...
                if os.path.exists("Abort_Message"):  # We need to stop RIGHT NOW
                    if (
                        self.crashtolerant
                    ):  # get out right now before the cleanup routines start
                ...
                os.system(
                    "echo '%02.6f  %02.6f'>>%s/balance.log" % (sb, tb, self.workdir)
                )

                if timelimit:
                    avgyear = (
                        self._checktimes()
                    )  # get how long it took to run each year
                    os.system(
                        "echo '%1.3f minutes'>>%s/runtimes.log"
                        % (avgyear[-1], self.workdir)
                    )
                    currentyears = np.loadtxt(
                        "%s/runtimes.log" % self.workdir,
                        usecols=[
                            0,
                        ],
                    )
                    ...
                    runlimit = min(
                        runstart + int(timelimit // currentavgyear), ogrunlimit
                    )
                    crunlimit = min(
                        int(timelimit // currentavgyear), ogrunlimit - runstart
                    )
                    # options for the runlimit are N0+T/tau, where tau is avg year
                    os.system(
                        "echo 'limit to %d years total; %d years this run'>>%s/limits.log"
                        % (runlimit, crunlimit, self.workdir)
                    )
    ...
    def gethistory(self, key="ts", mean=True, layer=-1):
        ...
        for n in range(0, len(files)):
            if "_metadata" not in files[n]:
                ...
                dd[n] = gcmt.spatialmath(
                    variable, lon=lon, lat=lat, mean=mean, radius=self.radius
                )
                ncd.close()
        return dd
    ...
    def _isbalanced(self, threshold=5.0e-4, baseline=50):
        ...
        if self.currentyear < baseline:  # Run for minimum of baseline years
            ...
            for n in range(9, len(sbalance)):
                savgs.append(
                    abs(np.mean(sbalance[n - 9 : n + 1]))
                )  # 10-year average energy balance
                tavgs.append(abs(np.mean(toabalance[n - 9 : n + 1])))
            sslopes = []
            tslopes = []
            for n in range(
                4, len(savgs)
            ):  # 5-baseline slopes in distance from energy balance
            ...
            os.system(
                "echo '%02.8f  %02.8f'>>%s/slopes.log"
                % (savgslope, tavgslope, self.workdir)
            )
            if (
                savgslope < threshold and tavgslope < threshold
            ):  # Both TOA and Surface are changing at average
                ...
                if postprocess:
                    try:
                        ...
                        if self.highcadence["toggle"]:
                            ...
                            os.system(
                                "mv %s*%s highcadence/" % (hcname, self.extension)
                            )
                    except Exception as e:
                        failed_postprocess = True
                        if self.crashtolerant or self.outputfaulttolerant:
    ...
    def cfgpostprocessor(
        self,
        ftype="regular",
        extension=".npz",
        namelist=None,
        variables=list(pyburn.ilibrary.keys()),
        mode="grid",
        zonal=False,
        substellarlon=180.0,
        physfilter=False,
        ...
        **kwargs : keyword arguments
        ...
        try:
            ...
            if variables is None and self._configuredpostprocessor[ftype]:
                pyburn.postprocess(
                    inputfile,
                    inputfile + self.extensions[ftype],
                    logfile=log,
                    radius=self.radius,
                ...
                if ftype != "regular":
                    ...
                    if "timeaverage" not in kwargs:
                        kwargs["timeaverage"] = self.postprocessordefaults[ftype][
                            "timeaverage"
                        ]
            ...
            if crashifbroken:
                if not self.recursecheck:
                    if self.integritycheck("%s%s" % (inputfile, extension)):
                        ...
                        raise RuntimeError(
                            "Error writing output to %s%s; " % (inputfile, extension)
                            + "log written to %s" % log
                        )
                else:
                    raise RuntimeError(
                ...
                print(
                    "Error writing output to %s%s; log written to %s"
                    % (inputfile, extension, log)
                )
                raise RuntimeError("Going to stop here just in case......")
            return 0
    ...
    def integritycheck(
        self, ncfile
    ):  # MUST pass an output archive that contains surface temperature
        ...
        if not os.path.exists(
            ncfile
        ):  # If the specified output file does not exist, create it
            if not self.recursecheck:
        ...
        if ioe:
            ...
            try:
                ...
                raise RuntimeError(
                    "Output is missing surface temperature; check logs for errors"
                )
    ...
    def finalize(self, outputdir, allyears=False, keeprestarts=False, clean=True):
        ...
        if allyears:
            ...
            os.system(
                "cp %s/MOST*%s %s/" % (self.workdir, self.extension, self.modelname)
            )
            if self.snapshots:
                os.system(
                    "cp -r %s/snapshots %s/snapshots" % (self.workdir, self.modelname)
                )
            if self.highcadence["toggle"]:
                os.system(
                    "cp -r %s/highcadence %s/highcadence"
                    % (self.workdir, self.modelname)
                )
            ...
            if self.extension == ".npz" or self.extension == ".npy":
                metaoutputs = sorted(
                    glob.glob("%s/MOST*metadata%s" % (self.workdir, self.extension))
                )
                tmpoutputs = sorted(
                    glob.glob("%s/MOST*%s" % (self.workdir, self.extension))
                )
            ...
            if self.extension == ".npz" or self.extension == ".npy":
                os.system(
                    "cp %s %s%s"
                    % (metaoutputs[-1], self.modelname + "_metadata", self.extension)
                )
            ...
            if self.snapshots:
                if self.extension == ".npz" or self.extension == ".npy":
                    metasnps = sorted(
                        glob.glob(
                            "%s/*SNAP*metadata%s" % (self.workdir, self.extension)
                        )
                    )
                    tmpsnps = sorted(
                        glob.glob("%s/snapshots/*%s" % (self.workdir, self.extension))
                    )
                ...
                os.system(
                    "cp %s %s_snapshot%s" % (snps[-1], self.modelname, self.extension)
                )
                if self.extension == ".npz" or self.extension == ".npy":
                    os.system(
                        "cp %s %s_snapshot_metadata%s"
                        % (metasnps[-1], self.modelname, self.extension)
                    )
            if self.highcadence["toggle"]:
                if self.extension == ".npz" or self.extension == ".npy":
                    metahcs = sorted(
                        glob.glob(
                            "%s/highcadence/MOST*metadata%s"
                            % (self.workdir, self.extension)
                        )
                    )
                    tmphcs = sorted(
                        glob.glob(
                            "%s/highcadence/MOST*%s" % (self.workdir, self.extension)
                        )
                    )
                ...
                os.system(
                    "cp %s %s_highcadence%s" % (hcs[-1], self.modelname, self.extension)
                )
                if self.extension == ".npz" or self.extension == ".npy":
                    os.system(
                        "cp %s %s_highcadence_metadata%s"
                        % (metahcs[-1], self.modelname, self.extension)
                    )
            if keeprestarts:
    ...
    def get(self, year, snapshot=False, highcadence=False):
        ...
        if os.path.exists(self.workdir + "/" + name):
            ...
            raise RuntimeError(
                "Output file %s not found." % (self.workdir + "/" + name)
            )
    ...
    def transit(
        ...
        if filename is None:
            if snapshot and not highcadence:
                name = self.workdir + "/snapshots/MOST_SNAP_transit.%05d%s" % (
                    year,
                    self.extension,
                )
            elif highcadence and not snapshot:
                name = self.workdir + "/highcadence/MOST_HC_transit.%05d%s" % (
                    year,
                    self.extension,
                )
            else:
        ...
        atm, wvl, spectra, coords, weights, avgspectra = pRT.transit(
            ncd,
            times,
            gases_vmr,
            gascon=self.gascon,
            gravity=self.gravity,
            rplanet=self.radius * 6.371e3,
            h2o_lines=h2o_linelist,
            num_cpus=num_cpus,
            cloudfunc=cloudfunc,
            smooth=smooth,
            smoothweight=smoothweight,
    ...
    def image(
        ...
        if filename is None:
            if snapshot and not highcadence:
                name = self.workdir + "/snapshots/MOST_SNAP_image.%05d%s" % (
                    year,
                    self.extension,
                )
            elif highcadence and not snapshot:
                name = self.workdir + "/highcadence/MOST_HC_image.%05d%s" % (
                    year,
                    self.extension,
                )
            else:
        ...
        if debug:
            (
                ...
                gravity=self.gravity,
                Tstar=self.startemp,
                Rstar=self.starradius,
                orbdistances=orbdistances,
                num_cpus=num_cpus,
                cloudfunc=cloudfunc,
                smooth=smooth,
                smoothweight=smoothweight,
            ...
            output = pRT.save(
                name,
                {
                    "wvl": wvl,
                    "time": times,
                    "star": atm.stellar_intensity * 1e6,
                    "images": spectra,
                    "colors": colors,
                    "lat": lat,
                    "lon": lon,
            ...
            atm, wvl, spectra, colors, lon, lat, avgspectra = pRT.image(
                ncd,
                times,
                gases_vmr,
                obsv_coords,
    ...
    def inspect(
        ...
        if not domulti:
            ...
            for kyear in range(1, len(year)):
                ...
                var = np.append(
                    var, ncd.variables[variable][:], axis=0
        ...
        if (
            ...
            if not savg and not tavg:
                ...
                if (
                    type(layer) != type(None) and len(var.shape) == 4
                ):  # 3D spatial array, plus time
                    # We're going to get a scalar; user has specified a level
                    return gcmt.spatialmath(
                        var, lat=lat, lon=lon, ignoreNaNs=ignoreNaNs, lev=layer
                    )
                    ...
                    for l in range(len(lev)):
                        output[l] = gcmt.spatialmath(
                            var, lat=lat, lon=lon, ignoreNaNs=ignoreNaNs, lev=l
                        )
                    return output
                elif len(var.shape) == 3:  # 2D spatial array, plus time
                    return gcmt.spatialmath(
                        var, lat=lat, lon=lon, ignoreNaNs=ignoreNaNs
                    )
            else:
                if (
                    type(layer) != type(None) and len(var.shape) == 4
                    ...
                    for t in range(len(output)):
                        output[t] = gcmt.spatialmath(
                            var, lat=lat, lon=lon, time=t, ignoreNaNs=ignoreNaNs
                        )
                    return output
                else:
                    return -1
    ...
    def _crash(self):
        ...
        raise RuntimeError(
            "ExoPlaSim has crashed or begun producing garbage. All working files have been moved to %s_crashed/"
            % (os.getcwd() + "/" + self.modelname)
        )
    ...
    def configure(
        ...
        meananomaly0=None,
        year=None,
        glaciers={"toggle": False, "mindepth": 2.0, "initialh": -1.0},
        restartfile=None,
        gravity=None,
        radius=None,
        eccentricity=None,
        ...
        if starspec:
            if starspec[0] != "/" or starspec[:2] not in [
                "A:",
                "B:",
                "C:",
                "D:",
                ...
                if os.path.isfile(starspec):  # found the file
                    ...
                    raise OSError(
                        f"Error: {starspec} not found. Try providing an absolute path."
                    )
            if not os.path.isfile(starspec):
                raise OSError(f"Error: {starspec} not found.")
            if not os.path.isfile("%s_hr.dat" % (starspec[:-4])):
                raise OSError(
                    "Error: %s_hr.dat not found; both %s and %s_hr.dat must be present."
                    % (starspec[:-4], starspec, starspec[:-4])
                )
            self._edit_namelist("radmod_namelist", "NSTARFILE", "1")
            self._edit_namelist("radmod_namelist", "STARFILE", "'%s'" % starspec)
            self._edit_namelist(
                "radmod_namelist", "STARFILEHR", "'%s_hr.dat'" % (starspec[:-4])
            )
        self.starspec = starspec
        if aerofile is not None:
            sourcedir = "/".join(__file__.split("/")[:-1])
            if f"{sourcedir}/hazeconstants/{aerofile}" in glob.glob(
                f"{sourcedir}/hazeconstants/*.dat"
            ):
                os.system(f"cp {sourcedir}/hazeconstants/{aerofile} {self.workdir}/")
            elif f"{sourcedir}/hazeconstants/{aerofile}.dat" in glob.glob(
                f"{sourcedir}/hazeconstants/*.dat"
            ):
                os.system(
                    f"cp {sourcedir}/hazeconstants/{aerofile}.dat {self.workdir}/"
                )
            if aerofile[-4:] == ".dat":
                aerofile = aerofile[:-4]
            self.aerorad = True
        ...
        if pressure:
            ...
            ):  # User has specified a different pressure than sum of gas pressures
                self.pressure = pressure
        ...
        self._edit_namelist(
            "plasim_namelist", "PSURF", str(self.pressure * 1.0e5)
        )  # Pa
        ...
        self._edit_namelist("plasim_namelist", "NSTPW", str(nstpw if nstpw else int(7200 // int(timestep))))
        ...
        if year:
            ...
            self.stepsperyear = 360.0 * 1440.0 / self.timestep
        self.sidyear = year
        if rotationperiod != 1.0:
            self._edit_namelist(
                "planet_namelist", "ROTSPD", str(1.0 / float(rotationperiod))
            )
            self._edit_namelist(
                "plasim_namelist",
                "N_DAYS_PER_YEAR",
        ...
        if qdiffusion:
            self._edit_namelist(
                "plasim_namelist", "TDISSQ", "%d*%f" % (self.layers, qdiffusion)
            )
        self.qdiffusion = qdiffusion
        if tdiffusion:
            self._edit_namelist(
                "plasim_namelist", "TDISST", "%d*%f" % (self.layers, tdiffusion)
            )
        self.tdiffusion = tdiffusion
        if zdiffusion:
            self._edit_namelist(
                "plasim_namelist", "TDISSZ", "%d*%f" % (self.layers, zdiffusion)
            )
        self.zdiffusion = zdiffusion
        if ddiffusion:
            self._edit_namelist(
                "plasim_namelist", "TDISSD", "%d*%f" % (self.layers, ddiffusion)
            )
        self.ddiffusion = ddiffusion
        if diffusionpower:
            self._edit_namelist(
                "plasim_namelist", "NDEL", "%d*%d" % (self.layers, diffusionpower)
            )
        self.diffusionpower = diffusionpower

        self.glaciers = glaciers
        self._edit_namelist(
            "glacier_namelist", "NGLACIER", str(self.glaciers["toggle"] * 1)
        )
        self._edit_namelist(
            "glacier_namelist", "GLACELIM", str(self.glaciers["mindepth"])
        )
        self._edit_namelist(
            "glacier_namelist", "ICESHEETH", str(self.glaciers["initialh"])
        )
        if self.glaciers["initialh"] > 0:
            os.system(
                "rm %s/*174.sra %s/*1740.sra %s/*210.sra %s/*232.sra"
                % tuple(
                    [
                        self.workdir,
                    ]
        ...
        self._edit_namelist(
            "radmod_namelist", "NSIMPLEALBEDO", str((not twobandalbedo) * 1)
        )
        ...
        if (
            oceanzenith == "lambertian"
            or oceanzenith == "Lambertian"
            or oceanzenith == "uniform"
        ):
            self._edit_namelist("radmod_namelist", "NECHAM", "0")
            self._edit_namelist("radmod_namelist", "NECHAM6", "0")
        if (
            oceanzenith == "default"
            or oceanzenith == "plasim"
            or oceanzenith == "ECHAM-3"
        ):
        ...
        if startemp is None:
            ...
            if wetsoil:
                print(
                    "Warning: wetsoil=True cannot be used with two-band albedos. Since you have a stellar temperature set, wetsoil is being set to False."
                )
        if soilwatercap:
        ...
        self._edit_namelist(
            "landmod_namelist", "DSOILZ", ",".join(self.dzsoils.astype(str))
        )
        ...
        if stratosphere:
            ...
            if modeltop:
                self._edit_namelist(
                    "plasim_namelist", "PTOP2", str(modeltop * 100.0)
                )  # convert hPa->Pa
            if tropopause:
            ...
            if modeltop:
                self._edit_namelist(
                    "plasim_namelist", "PTOP", str(modeltop * 100.0)
                )  # convert hPa->Pa
        ...
        self._edit_namelist(
            "plasim_namelist", "HCINTERVAL", str(highcadence["interval"])
        )
        ...
        if landmap:
            os.system(
                "cp %s %s/N%03d_surf_0172.sra" % (landmap, self.workdir, self.nlats)
            )
        if topomap:
            os.system(
                "cp %s %s/N%03d_surf_0129.sra" % (topomap, self.workdir, self.nlats)
            )
        ...
        if stormcapture["toggle"]:
            ...
            for param in stormcapture:
                if param != "toggle":
                    self._edit_namelist(
                        "hurricane_namelist", param, str(stormcapture[param])
                    )
    ...
    def loadconfig(self, configfile):
        ...
        for item in stormdict:
            ...
            if (
                parts[0] == "toggle"
                or parts[0] == "NKTRIGGER"
                or parts[0] == "SIZETHRESH"
                or parts[0] == "ENDTHRESH"
                or parts[0] == "MINSTORMLEN"
                or parts[0] == "MAXSTORMLEN"
            ):
        ...
        self.configure(
            ...
            gascon=gascon,
            pressure=pressure,
            pressurebroaden=pressurebroaden,
            vtype=vtype,
            rotationperiod=rotationperiod,
            synchronous=synchronous,
            year=year,
            top_restoretime=restim,
            runsteps=runsteps,
            substellarlon=substellarlon,
            ...
            obliquity=obliquity,
            lonvernaleq=lonvernaleq,
            fixedorbit=fixedorbit,
            orography=orography,
            seaice=seaice,
            co2weathering=co2weathering,
            evolveco2=evolveco2,
            physicsfilter=physicsfilter,
            filterkappa=filterkappa,
            filterpower=filterpower,
            filterLHN0=filterLHN0,
            diffusionwaven=diffusionwaven,
            qdiffusion=qdiffusion,
            tdiffusion=tdiffusion,
            zdiffusion=zdiffusion,
            ddiffusion=ddiffusion,
            diffusionpower=diffusionpower,
            desync=desync,
            erosionsupplylimit=erosionsupplylimit,
            outgassing=outgassing,
            snowicealbedo=snowicealbedo,
            twobandalbedo=twobandalbedo,
            maxsnow=maxsnow,
            soilalbedo=soilalbedo,
            oceanalbedo=oceanalbedo,
            oceanzenith=oceanzenith,
            wetsoil=wetsoil,
            soilwatercap=soilwatercap,
            aquaplanet=aquaplanet,
            vegetation=vegetation,
            vegaccel=vegaccel,
            nforestgrowth=nforestgrowth,
            initgrowth=initgrowth,
            initstomcond=initstomcond,
            initrough=initrough,
            initsoilcarbon=initsoilcarbon,
            initplantcarbon=initplantcarbon,
            ...
            stormclim=stormclim,
            nstorms=nstorms,
            stormcapture=stormcapture,
            topomap=topomap,
            tlcontrast=tlcontrast,
            otherargs=otherargs,
            glaciers=glaciers,
            threshold=threshold,
            keplerian=keplerian,
    ...
    def modify(self, **kwargs):
        ...
        if "timestep" in kwargs.keys():
            ...
            self._edit_namelist(
                "plasim_namelist", "NSTPW", str(int(7200 // int(self.timestep)))
            )
        ...
        for key, value in kwargs.items():
            ...
            if key == "starspec":
                ...
                if starspec:
                    ...
                    self._edit_namelist("radmod_namelist", "NSTARFILE", "1")
                    self._edit_namelist(
                        "radmod_namelist", "STARFILE", "'%s'" % starspec
                    )
                    self._edit_namelist(
                        "radmod_namelist", "STARFILEHR", "'%s_hr.dat'" % (starspec[:-4])
                    )
            ...
            if key == "pressurebroaden":
                ...
                self._edit_namelist(
                    "radmod_namelist", "NPBROADEN", str(self.pressurebroaden * 1)
                )
            if key == "vtype":
                self.vtype = value
            ...
            if key == "year":
                ...
                if self.sidyear:
                    self._edit_namelist(
                        "plasim_namelist", "N_DAYS_PER_YEAR", str(int(self.sidyear))
                    )
                    self._edit_namelist(
                        "planet_namelist", "SIDEREAL_YEAR", str(self.sidyear * 86400.0)
                    )
            if key == "rotationperiod":
                ...
                if self.rotationperiod != 1.0:
                    ...
                    self._edit_namelist(
                        ...
                        "N_DAYS_PER_YEAR",
                        str(
                            max(int(360.0 / float(self.rotationperiod) / 12 + 0.5), 1)
                            * 12
                        ),
                    )
                    # if value>80.0:
                    ...
                    if "runsteps" in kwargs.keys():
                        if kwargs["runsteps"] is None:
                            nsteps = max(
                                1, int(round(360.0 * 1440.0 / self.timestep + 0.49999))
                            )
                            self._edit_namelist(
                                "plasim_namelist", "N_RUN_STEPS", str(nsteps)
                            )
                    else:
                        nsteps = max(
                            1, int(round(360.0 * 1440.0 / self.timestep + 0.49999))
            ...
            if key == "runsteps":
                ...
                if self.runsteps is not None:
                    self._edit_namelist(
                        "plasim_namelist", "N_RUN_STEPS", str(self.runsteps)
                    )
            ...
            if key == "synchronous":
                ...
                self._edit_namelist(
                    "radmod_namelist", "NFIXED", str(self.synchronous * 1)
                )
            if key == "substellarlon":
                self.substellarlon = value
                self._edit_namelist(
                    "plasim_namelist", "FIXEDLON", str(self.substellarlon)
                )
            if key == "desync":
                self.desync = value
            ...
            if key == "keplerian":
                ...
                self._edit_namelist(
                    "planet_namelist", "NGENKEPLERIAN", str(self.keplerian * 1)
                )
            if key == "meananomaly0":
                self.meananomaly0 = value
                if self.meanomaly0 is not None:
                    self._edit_namelist(
                        "planet_namelist", "MEANANOMALY0", str(self.meananomaly0)
                    )
            if key == "tlcontrast":
            ...
            if key == "restartfile":
                ...
                if self.restartfile:
                    os.system(
                        "cp %s %s/plasim_restart" % (self.restartfile, self.workdir)
                    )
                else:
                    os.system("rm %s/plasim_restart" % self.workdir)
            if key == "gravity":
            ...
            if key == "radius":
                self.radius = value
                self._edit_namelist(
                    "planet_namelist", "PLARAD", str(self.radius * 6371220.0)
                )
                self._edit_postnamelist(
                    "example.nl", "radius", str(self.radius * 6371220.0)
                )
                self._edit_postnamelist(
                    "snapshot.nl", "radius", str(self.radius * 6371220.0)
                )
            if key == "eccentricity":
            ...
            if key == "fixedorbit":
                ...
                self._edit_namelist(
                    "planet_namelist", "NFIXORB", str(self.fixedorbit * 1)
                )
            ...
            if key == "orography":
                ...
                self._edit_namelist(
                    "glacier_namelist", "NGLACIER", str((self.orography != 1) * 1)
                )
            if key == "seaice":
                self.seaice = value
            ...
            if key == "co2weathering":
                ...
                self._edit_namelist(
                    "carbonmod_namelist", "NCARBON", str(self.co2weathering * 1)
                )
            if key == "evolveco2":
                self.evolveco2 = value
                self._edit_namelist(
                    "carbonmod_namelist", "NCO2EVOLVE", str(self.evolveco2 * 1)
                )
            ...
            if key == "outgassing":
                ...
                self._edit_namelist(
                    "carbonmod_namelist", "VOLCANCO2", str(self.outgassing)
                )
            ...
            if key == "filterkappa":
                ...
                self._edit_namelist(
                    "plasim_namelist", "FILTERKAPPA", str(self.filterkappa)
                )
            if key == "filterpower":
                self.filterpower = value
                self._edit_namelist(
                    "plasim_namelist", "NFILTEREXP", str(self.filterpower)
                )
            if key == "filterLHN0":
                self.filterLHN0 = value
                self._edit_namelist(
                    "plasim_namelist", "LANDHOSKN0", str(self.filterLHN0)
                )
            if key == "diffusionwaven":
                self.diffusionwaven = value
                if value:
                    self._edit_namelist(
                        "plasim_namelist", "NHDIFF", str(self.diffusionwaven)
                    )
                else:
            ...
            if key == "qdiffusion":
                ...
                if value:
                    self._edit_namelist(
                        "plasim_namelist",
                        "TDISSQ",
                        "%d*%f" % (self.layers, self.qdiffusion),
                    )
                else:
            ...
            if key == "tdiffusion":
                ...
                if value:
                    self._edit_namelist(
                        "plasim_namelist",
                        "TDISST",
                        "%d*%f" % (self.layers, self.tdiffusion),
                    )
                else:
            ...
            if key == "zdiffusion":
                ...
                if value:
                    self._edit_namelist(
                        "plasim_namelist",
                        "TDISSZ",
                        "%d*%f" % (self.layers, self.zdiffusion),
                    )
                else:
            ...
            if key == "ddiffusion":
                ...
                if value:
                    self._edit_namelist(
                        "plasim_namelist",
                        "TDISSD",
                        "%d*%f" % (self.layers, self.ddiffusion),
                    )
                else:
            ...
            if key == "diffusionpower":
                ...
                if value:
                    self._edit_namelist(
                        "plasim_namelist",
                        "NDEL",
                        "%d*%d" % (self.layers, self.diffusionpower),
                    )
                else:
            ...
            if key == "snowicealbedo":
                ...
                if type(self.snowicealbedo) != type(None):
                    ...
                    self._edit_namelist("seamod_namelist", "ALBICE", alb)
                    self._edit_namelist(
                        "seamod_namelist", "DICEALBMN", "%s,%s" % (alb, alb)
                    )
                    self._edit_namelist(
                        "seamod_namelist", "DICEALBMX", "%s,%s" % (alb, alb)
                    )
                    self._edit_namelist(
                        "landmod_namelist", "DSNOWALBMN", "%s,%s" % (alb, alb)
                    )
                    self._edit_namelist(
                        "landmod_namelist", "DSNOWALBMN", "%s,%s" % (alb, alb)
                    )
                    self._edit_namelist(
                        "landmod_namelist", "DGLACALBMN", "%s,%s" % (alb, alb)
                    )
                    self._edit_namelist(
                        "landmod_namelist", "DSNOWALB", "%s,%s" % (alb, alb)
                    )
                else:
            ...
            if key == "soilalbedo":
                ...
                if type(self.soilalbedo) != type(None):
                    ...
                    self._edit_namelist("landmod_namelist", "ALBLAND", alb)
                    self._edit_namelist(
                        "landmod_namelist", "DGROUNDALB", "%s,%s" % (alb, alb)
                    )
                else:
            ...
            if key == "oceanalbedo":
                ...
                if type(self.oceanalbedo) != type(None):
                    ...
                    self._edit_namelist("seamod_namelist", "ALBSEA", alb)
                    self._edit_namelist(
                        "seamod_namelist", "DOCEANALB", "%s,%s" % (alb, alb)
                    )
                else:
            ...
            if key == "oceanzenith":
                ...
                if (
                    self.oceanzenith == "lambertian"
                    or self.oceanzenith == "Lambertian"
                    or self.oceanzenith == "uniform"
                ):
                    self._edit_namelist("radmod_namelist", "NECHAM", "0")
                    self._edit_namelist("radmod_namelist", "NECHAM6", "0")
                if (
                    self.oceanzenith == "default"
                    or self.oceanzenith == "plasim"
                    or self.oceanzenith == "ECHAM-3"
                ):
                    self._edit_namelist("radmod_namelist", "NECHAM", "1")
                    self._edit_namelist("radmod_namelist", "NECHAM6", "0")
            ...
            if key == "wetsoil":
                if "startemp" in kwargs and kwargs["startemp"] is not None:
                    ...
                    if value:
                        print(
                            "Warning: wetsoil=True cannot be used in conjunction with startemp. Setting wetsoil=False."
                        )
                else:
                    self.wetsoil = value
                    self._edit_namelist(
                        "landmod_namelist", "NWETSOIL", str(self.wetsoil * 1)
                    )
            if key == "soilwatercap":
                self.soilwatercap = value
                if self.soilwatercap:
                    self._edit_namelist(
                        "landmod_namelist", "WSMAX", str(self.soilwatercap)
                    )
                    os.system("rm %s/*0229.sra" % self.workdir)
                else:
            ...
            if key == "soilsaturation":
                ...
                if type(self.soilsaturation) != type(None):
                    self._edit_namelist("landmod_namelist", "NWATCINI", "1")
                    self._edit_namelist(
                        "landmod_namelist", "DWATCINI", str(self.soilsaturation)
                    )
                    os.system("rm %s/*0229.sra" % self.workdir)
                else:
            ...
            if key == "nforestgrowth":
                ...
                self._edit_namelist(
                    "vegmod_namelist", "FORGROW", str(self.nforestgrowth)
                )
            if key == "initgrowth":
            ...
            if key == "initstomcond":
                ...
                self._edit_namelist(
                    "vegmod_namelist", "RINIDSC", str(self.initstomcond)
                )
            if key == "initrough":
            ...
            if key == "initsoilcarbon":
                ...
                self._edit_namelist(
                    "vegmod_namelist", "RINISOIL", str(self.initsoilcarbon)
                )
            if key == "initplantcarbon":
                self.initplantcarbon = value
                self._edit_namelist(
                    "vegmod_namelist", "RINIVEG", str(self.initplantcarbon)
                )
            ...
            if key == "drycore":
                self.drycore = value
                self._edit_namelist(
                    "fluxmod_namelist", "NEVAP", str((not self.drycore) * 1)
                )
            ...
            if key == "ozone":
                ...
                if self.ozone is False or self.ozone is True:
                    ...
                    self._edit_namelist("radmod_namelist", "NO3", "1")
                    self._edit_namelist(
                        "radmod_namelist", "A0O3", str(self.ozone["amount"])
                    )
                    self._edit_namelist(
                        "radmod_namelist", "A1O3", str(self.ozone["varlat"])
                    )
                    self._edit_namelist(
                        "radmod_namelist", "ACO3", str(self.ozone["varseason"])
                    )
                    self._edit_namelist(
                        "radmod_namelist", "TOFFO3", str(self.ozone["seasonoffset"])
                    )
                    self._edit_namelist(
                        "radmod_namelist", "BO3", str(self.ozone["height"])
                    )
                    self._edit_namelist(
                        "radmod_namelist", "CO3", str(self.ozone["spread"])
                    )
            ...
            if key == "writefrequency":
                ...
                if self.writefrequency:
                    self._edit_namelist(
                        "plasim_namelist", "NWPD", str(self.writefrequency)
                    )
                else:
            ...
            if key == "highcadence":
                ...
                self._edit_namelist(
                    "plasim_namelist", "NHCADENCE", str(self.highcadence["toggle"])
                )
                self._edit_namelist(
                    "plasim_namelist", "HCSTARTSTEP", str(self.highcadence["start"])
                )
                self._edit_namelist(
                    "plasim_namelist", "HCENDSTEP", str(self.highcadence["end"])
                )
                self._edit_namelist(
                    "plasim_namelist", "HCINTERVAL", str(self.highcadence["interval"])
                )
            ...
            if key == "stormclim":
                ...
                if self.stormclim:
                    self._edit_namelist("hurricane_namelist", "NSTORMDIAG", "1")
                    self._add_postcodes(
                        "example.nl", [322, 323, 324, 325, 326, 327, 328, 329]
                    )
                    self._add_postcodes(
                        "snapshot.nl", [322, 323, 324, 325, 326, 327, 328, 329]
                    )
                else:
            ...
            if key == "nstorms":
                self.nstorms = value
                self._edit_namelist(
                    "hurricane_namelist", "NSTORMS", str(self.int(nstorms))
                )
            if key == "stormcapture":
            ...
            if key == "aerorad":
                ...
                if "aerofile" not in kwargs and self.aerorad:
                    ...
                    if self.aerofile is None:
                        self.aerofile = "gj667_constants_500.dat"
                if self.aerorad:
                    if f"{sourcedir}/hazeconstants/{self.aerofile}" in glob.glob(
                        f"{sourcedir}/hazeconstants/*.dat"
                    ):
                        os.system(
                            f"cp {sourcedir}/hazeconstants/{self.aerofile} {self.workdir}/"
                        )
                    elif f"{sourcedir}/hazeconstants/{self.aerofile}.dat" in glob.glob(
                        f"{sourcedir}/hazeconstants/*.dat"
                    ):
                        os.system(
                            f"cp {sourcedir}/hazeconstants/{self.aerofile}.dat {self.workdir}/"
                        )
                    if self.aerofile[-4:] == ".dat":
                        self.aerofile = aerofile[:-4]
                    self.aerorad = True
                    self._edit_namelist(
                        "aero_namelist", "l_aerorad", str(self.aerorad * 1)
                    )
                    self._edit_namelist(
                        "aero_namelist", "aerofile", "'%s.dat'" % self.aerofile
                    )
                    self.aerofile += ".dat"
                else:
                    self._edit_namelist(
        ...
        if restim:
            ...
            self._edit_namelist("plasim_namelist", "NSPONGE", "1")
            self._edit_namelist(
                "plasim_namelist", "DAMPSP", "%" % self._top_restoretime
            )
        ...
        if changeatmo:
            ...
            if self.stratosphere:
                ...
                if self.modeltop:
                    self._edit_namelist(
                        "plasim_namelist", "PTOP2", str(self.modeltop * 100.0)
                    )
                    # convert hPa->Pa
                if self.tropopause:
                    self._edit_namelist(
                        "plasim_namelist", "PTOP", str(self.tropopause * 100.0)
                    )
            else:
                if self.modeltop:
                    self._edit_namelist(
                        "plasim_namelist", "PTOP", str(self.modeltop * 100.0)
                    )
                    # convert hPa->Pa
        ...
        if changeland:
            ...
            if self.landmap:
                os.system(
                    "cp %s %s/N%03d_surf_0172.sra"
                    % (self.landmap, self.workdir, self.nlats)
                )
            if self.topomap:
                os.system(
                    "cp %s %s/N%03d_surf_0129.sra"
                    % (self.topomap, self.workdir, self.nlats)
                )
...
class TLaquaplanet(Model):
    ...
    def configure(
        ...
        **kwargs,
        ...
        super(TLaquaplanet, self).configure(
            synchronous=True,
            fixedorbit=True,
            aquaplanet=True,
            eccentricity=eccentricity,
...
class TLlandplanet(
    Model
):  # Default will be ZERO soil water; set soilsaturation if you want any
    ...
    def configure(
        ...
        **kwargs,
        ...
        super(TLlandplanet, self).configure(
            synchronous=True,
            fixedorbit=True,
            desertplanet=True,
            eccentricity=eccentricity,
...
class Earthlike(Model):
    ...
    def configure(self, timestep=45.0, snapshots=480, vtype=4, modeltop=50.0, **kwargs):
        super(Earthlike, self).configure(
            vtype=vtype,
            modeltop=modeltop,
            timestep=timestep,
            snapshots=snapshots,
