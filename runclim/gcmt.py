...
class _Constants:
    def __init__(self):
        ...
        self.Rearth_auth = (
            6.3710072e6  # Earth authalic radius (same area as reference ellipsoid)
        )
        self.Rearth_vol = (
            6.3710008e6  # Earth volumetric radius (same volume as reference ellipsoid)
        )
        self.Rearth_rec = (
            6.367445e6  # Earth rectifying radius (same polar cross section perimeter)
        )
...
def blackbody(wavelengths, temperature):
    ...
    bb = (
        ...
        * consts.c**2
        / y**5
        * 1
        / (np.exp(consts.h * consts.c / (y * consts.kb * T)) - 1)
    )  # W sr-1 m-3
    ...
    def __getitem__(self, key, overridebuffer=False):
        ...
        if key not in self.dbuffer:
            ...
            if not overridebuffer:
                ...
                self.dbufferkeys.append(
                    key
                )  # Move this key to the end so it's the last to be removed.
        return data
    ...
    def __setitem__(self, key, value):
        ...
        if key in self.filetree:
            ...
            np.savetxt(
                ...
                header=(
                    ",".join(np.array(value.shape).astype(str))
                    + ",|||,"
                    + ",".join([key, key, "user", "-333"])
                ),
                delimiter=",",
            )
            if self.tarball:
                if (
                    "tar.gz" in self.archive
                    or "tar.bz2" in self.archive
                    or "tar.xz" in self.archive
                ):
                    with tarfile.open(
                        self.archive, "w:%s" % self.archive.split(".")[-1]
                    ) as tarball:
                        for var in members:
    ...
    def __init__(self, filename, csvbuffersize=1):
        ...
        if fileparts[-1] == "nc":
            ...
            for var in self.variables:
                self.metadata[var] = {}
                try:
                    self.metadata[var]["standard_name"] = self.variables[
                        var
                    ].standard_name
                except:
        ...
        elif fileparts[-1] == "tar" or fileparts[-2] + "." + fileparts[-1] in (
            "tar.gz",
            "tar.bz2",
            "tar.xz",
        ):
...
def make2d(
    ...
    if time:
        try:
            ...
            raise UnitError(
                "You have probably passed a float time to a variable with no "
        ...
        if lev is not None:
            ...
            if type(lat) == int:
                ...
                if longitudes is None or latitudes is None:
                    raise Exception(
                        "You must provide latitudes and longitudes as 1D arrays."
                    )
                variable = latsum(
                    variable,
                    latitudes,
            ...
            if type(lon) == int:
                ...
                gradlat = np.gradient(np.sin(latitudes * np.pi / 180.0))
                for lt in range(len(latitudes)):
                    newvar[:, lt] = lonsum(
                        variable, longitudes, dsinlat=gradlat[lt], radius=radius
                    )
                variable = newvar
            elif lon == "mean":
            ...
            raise DimensionError(
                "Inappropriate or insufficient dimensional constraints"
            )
    ...
    if time != "all":
        ...
        for jlat in range(0, len(lt)):
            for jlon in range(0, len(ln)):
                ...
                darea[jlat, jlon] = abs(
                    np.sin(lt1[jlat]) - np.sin(lt1[jlat + 1])
                ) * abs(dln)
        ...
        outvariable = np.zeros(
            [
                ntimes,
            ]
            + list(variable_.shape)
        )
        outvariable[0, ...] = variable_[:]
        for n in range(1, ntimes):
            outvariable[n, ...] = make2d(
                variable,
                time=n,
                lev=lev,
                ignoreNaNs=ignoreNaNs,
...
def latmean(variable, latitudes):
    ...
    if len(variable.shape) == 1:
        ...
        return np.nansum(variable * darea[:, np.newaxis], axis=0) / np.nansum(
            darea[:, np.newaxis] * np.ones(variable.shape), axis=0
        )
    elif len(variable.shape) == 3:
        return np.nansum(
            variable * darea[np.newaxis, :, np.newaxis], axis=1
        ) / np.nansum(
            darea[np.newaxis, :, np.newaxis] * np.ones(variable.shape), axis=1
        )
    elif len(variable.shape) == 4:
        return np.nansum(
            variable * darea[np.newaxis, np.newaxis, :, np.newaxis], axis=2
        ) / np.nansum(
            darea[np.newaxis, np.newaxis, :, np.newaxis] * np.ones(variable.shape),
            axis=2,
        )
    else:
        raise DimensionError(
            "Variable must have 4 or fewer dimensions. Latitude should be the second-from the right-most dimension if there are 2 or more dimensions."
        )
...
def latsum(variable, latitudes, dlon=360.0, radius=6.371e6):
    ...
    for jlat in range(0, len(latitudes)):
        darea[jlat] = (
            abs(np.sin(lt1[jlat]) - np.sin(lt1[jlat + 1])) * abs(dlon) * radius**2
        )
...
def cspatialmath(
    ...
    if file:
        ...
        lt = lat
    variable = make2d(
        variable, time=time, lev=lev, ignoreNaNs=ignoreNaNs, latitudes=lt, longitudes=ln
    )
...
def eqstream(file, radius=6.371e6, gravity=9.80665):
    ...
    pa = (
        dataset.variables["ps"][:, np.newaxis, :, :]
        * lev[np.newaxis, :, np.newaxis, np.newaxis]
        * 100.0
    )
    ...
    for nt in range(ntime):
        for jlat in range(nlat):
            for jlon in range(nlon):
                vadp[nt, :jlat, jlon] = cumulative_trapezoid(
                    va[nt, :, jlat, jlon], x=pa[nt, :, jlat, jlon], initial=0.0
                )
...
def adist(lon1, lat1, lon2, lat2):
    ...
    dist = np.arccos(
        np.sin(rt1) * np.sin(rt2) + np.cos(rt1) * np.cos(rt2) * np.cos(rn1 - rn2)
    )
        ...
        for j in range(nlat):
            ...
            if abs(dlat) > abs(lat).max():
                if polemethod != "nearest":
                    ...
                    tlvariable[..., j, i] = np.average(
                        variable[..., jj, :],
                        weights=distances,
                        axis=len(variable.shape) - 2,
                    )
                else:
                ...
                if not colat and not colon:
                    ...
                    if ii == 0:
                        ln1 = abs(
                            loncomparison[-1] - 360.0 - dlon
                        )  # if chosen, our indices will be -1 and 0
                        ln2 = loncomparison[1]  # if chosen, our indices will be 0 and 1
                    elif ii == nlon - 1:
                        ln1 = loncomparison[
                            ii - 1
                        ]  # if chosen, our indices will be nlon-2 and nlon-1
                        ln2 = abs(
                            360.0 - dlon
                        )  # if chosen, our indices will be nlon-1 and 0
                    else:
                    ...
                    tlvariable[..., j, i] = np.average(
                        np.array([variable[..., jj, ii], variable[..., jj, ni]]),
                        weights=[d1, d2],
                        axis=0,
                    )
                    ...
                    tlvariable[..., j, i] = np.average(
                        np.array([variable[..., jj, ii], variable[..., nj, ii]]),
                        weights=[d1, d2],
                        axis=0,
                    )
...
def eq2tl_uv(u, v, lon, lat, substellar=0.0):
    ...
    """
    lon_tl, lat_tl, uq_tl = eq2tl(
        u, lon, lat, substellar=substellar, polemethod="nearest"
    )
    lon_tl, lat_tl, vq_tl = eq2tl(
        v, lon, lat, substellar=substellar, polemethod="nearest"
    )
    ...
    ufactor = -np.cos(rtlats) / (
        np.sin(rqlats) * (1 + np.sin(rqlons) ** 2 / np.tan(rqlats) ** 2)
    )
...
def tlstream(dataset, radius=6371.0e3, gravity=9.80665, substellar=0.0):
    ...
    lon = dataset.variables["lon"][:]
    lat = dataset.variables["lat"][:]
    lon_TL, lat_TL, ua_TL, va_TL = eq2tl_uv(
        dataset.variables["ua"][:],
    ...
    for nt in range(ntime):
        for jlat in range(nlat):
            for jlon in range(nlon):
                vadp[nt, :, jlat, jlon] = cumulative_trapezoid(
                    va_TL[nt, :, jlat, jlon], x=pa[nt, :, jlat, jlon], initial=0.0
                )
...
def orthographic(
    ...
    if lon.min() < 0:  # we're in a -180 to 180 domain
        ...
        imap = np.roll(
            imap, -int(dl0 / np.diff(lon)[0]), axis=1
        )  # l0 should now be centered in the data array

    else:
        if l0 < 0:
            l0 += 360.0

        dl0 = l0 - 180.0  # l0-dl0 = 180
        l0 -= dl0
        ...
        for i in range(0, nx):
            ...
            if (ix**2 + jy**2) <= rad**2:
                ...
                if rho == 0:
                    ...
                    phi = np.arcsin(
                        np.cos(cc) * np.sin(p0) + jy * np.sin(cc) * np.cos(p0) / rho
                    )
                    lamb = l0 + np.arctan2(
                        ix * np.sin(cc),
                        (rho * np.cos(cc) * np.cos(p0) - jy * np.sin(cc) * np.sin(p0)),
                    )
                phi *= 180.0 / np.pi
                lamb *= 180.0 / np.pi
                    ...
                    if (
                        ...
                        try:
                            if dl == 0 and dp > 0:
                                ...
                                xymap[j, i] = (p2 - phi) / dp * fxy1 + (
                                    phi - p1
                                ) / dp * fxy2
                            elif dp == 0 and dl > 0:
                                fx1y = zmap[jlat, jlon1]
                                fx2y = zmap[jlat, jlon2]
                                xymap[j, i] = (l2 - lamb) / dl * fx1y + (
                                    lamb - l1
                                ) / dl * fx2y
                            elif dp == 0 and dl == 0:
                                xymap[j, i] = zmap[jlat, jlon]
                            else:
                                fxy1 = (l2 - lamb) / dl * zmap[jlat1, jlon1] + (
                                    lamb - l1
                                ) / dl * zmap[jlat1, jlon2]
                                fxy2 = (l2 - lamb) / dl * zmap[jlat2, jlon1] + (
                                    lamb - l1
                                ) / dl * zmap[jlat2, jlon2]
                                xymap[j, i] = (p2 - phi) / dp * fxy1 + (
                                    phi - p1
...
def load(filename, csvbuffersize=1):
    ...
    if type(csvbuffersize) == str:  # This is probably a legacy netcdf load mistake
        try:
            ...
            csvbuffersize = 1
    output = _Dataset(
        filename, csvbuffersize=csvbuffersize
    )  # Usually _Dataset calls load(), but _Dataset calls _loadnetcdf
