...
RH2O = 1000 * 1.380658e-23 * 6.0221367e23 / 18.0153
...
def _getEndian(fbuffer):
    ...
    if (
        tag > 1024
    ):  # If we get a huge number for what should be an 8-word header, flip the byte order
        if (
            endian == ">"
        ):  # We're in a Big-Endian system, and the buffer was written in little-Endian.
            endian = "<"
...
def _getknownwordlength(fbuffer, n, en, ml, mf):
    ...
    wordlength = (
        dtag // length
    )  # dtag tells us the length of the coming record in bytes, while length is the
...
def readrecord(fbuffer, n, en, ml, mf):
    ...
    if n < len(fbuffer):
        ...
        header = struct.unpack(
            en + headerlength * "i", fbuffer[n : n + headerlength * 4]
        )
        n += (
            headerlength * 4 + ml
        )  # Add one word for restatement of header length (for backwards seeking)
    ...
    while n < len(fbuffer):
        ...
        if header[0] == kcode:
            ...
            if not variable:
                variable = np.array(
                    struct.unpack(
                        en + datalength * fmt, fbuffer[n : n + datalength * wl]
                    )
                )
            else:
                variable = np.append(
...
def readallvariables(fbuffer):
    ...
    while n < len(fbuffer):
        ...
        kcode = str(header[0])
        if int(kcode) == 139:
            variables["time"].append(
                header[6]
            )  # nstep-nstep1 (timesteps since start of run)
...
def readfile(filename):
    ...
    for key in kcodes:
        data[key] = refactorvariable(
            variables[key], headers[key], ntimes=ntimes, nlev=nlevs
        )
...
def _transformvar(
    ...
    if mode == "grid":
        if (ntru + 1) * (ntru + 2) in variable.shape:  # spectral variable
            if len(variable.shape) == 3:  # Include lev
                ...
                spvar = np.asfortranarray(
                    np.transpose(
                        np.reshape(variable, (ntimes * nlevs, variable.shape[2]))
                    )
                )
                ...
                ntimes = variable.shape[0]
                spvar = np.asfortranarray(
                    np.transpose(np.reshape(variable, (ntimes, variable.shape[1])))
                )
        ...
        if not presync and "lat" in dims:
            lon, lat, tlgridvar = gcmt.eq2tl(
                gridvar, lon, lat, substellar=substellarlon, polemethod="interp"
            )  # fine bc all vectors are derived
        else:
            tlgridvar = gridvar
        ...
        if (ntru + 1) * (ntru + 2) in variable.shape:  # spectral variable
            ...
            if len(variable.shape) == 4:  # Include lev
                ...
                gpvar = np.asfortranarray(
                    np.transpose(np.reshape(variable, (ntimes * nlevs, nlat, nlon)))
                )
                dims = ("time", levd, "modes", "complex")
            elif len(variable.shape) == 2:  # column
                outvar = variable
                ...
                ntimes = variable.shape[0]
                gpvar = np.asfortranarray(
                    np.transpose(np.reshape(variable, (ntimes, nlat, nlon)))
                )
                dims = ("time", "modes", "complex")
            if "modes" in dims:
        ...
        if (ntru + 1) * (ntru + 2) in variable.shape:  # spectral variable
            ...
            if len(variable.shape) == 4:  # include lev
                ...
                gpvar = np.asfortranarray(
                    np.transpose(
                        np.reshape(
                            variable / 1.4142135623730951, (ntimes * nlevs, nlat, nlon)
                        )
                    )
                )
                fcshape = (ntimes, nlevs, nlat, nlon // 2, 2)
                ...
                gpvar = np.asfortranarray(
                    np.transpose(
                        np.reshape(variable / 1.4142135623730951, (ntimes, nlat, nlon))
                    )
                )
                fcshape = (ntimes, nlat, nlon // 2, 2)
                dims = ["time", "lat", "fourier", "complex"]
        ...
        if "lat" in dims:
            ...
            if len(rottlgridvar.shape) == 4:  # include lev
                ...
                gpvar = np.asfortranarray(
                    np.transpose(
                        np.reshape(
                            rottlgridvar / 1.4142135623730951,
                ...
                gpvar = np.asfortranarray(
                    np.transpose(
                        np.reshape(
                            rottlgridvar / 1.4142135623730951, (ntimes, nlat, nlon)
                        )
                    )
                )
                fcshape = (ntimes, nlat, nlon // 2, 2)
    ...
    if mode == "grid":
        if (ntru + 1) * (ntru + 2) in uvar.shape:  # spectral variable
            if len(uvar.shape) == 3:  # Include lev
                ...
                spuvar = np.asfortranarray(
                    np.transpose(np.reshape(uvar, (ntimes * nlevs, uvar.shape[2])))
                )
                spvvar = np.asfortranarray(
                    np.transpose(np.reshape(vvar, (ntimes * nlevs, vvar.shape[2])))
                )
                gridshape = (ntimes, nlevs, nlat, nlon)
                dims = ["time", levd, "lat", "lon"]
            else:
                ntimes = uvar.shape[0]
                spuvar = np.asfortranarray(
                    np.transpose(np.reshape(uvar, (ntimes, variable.shape[1])))
                )
                spvvar = np.asfortranarray(
                    np.transpose(np.reshape(vvar, (ntimes, variable.shape[1])))
                )
                gridshape = (ntimes, nlat, nlon)
                dims = ["time", "lat", "lon"]
            griduvar, gridvvar = pyfft.spvgp(
                spuvar, spvvar, rdcostheta, nlon, ntru, int(physfilter)
            )
        ...
        if (ntru + 1) * (ntru + 2) in uvar.shape:  # spectral variable
            if len(uvar.shape) == 3:  # Include lev
                ...
                ntimes = uvar.shape[0]
                spuvar = np.asfortranarray(
                    np.transpose(np.reshape(uvar, (ntimes, uvar.shape[1])))
                )
                spvvar = np.asfortranarray(
                    np.transpose(np.reshape(vvar, (ntimes, vvar.shape[1])))
                )
                gridshape = (ntimes, nlat, nlon)
                dims = ["time", "lat", "lon"]
            griduvar, gridvvar = pyfft.spvgp(
            ...
            if len(uvar.shape) == 3:
                ...
                dims = ["time", levd, "lat", "lon"]
        lon, lat, tlgriduvar, tlgridvvar = gcmt.eq2tl_uv(
            griduvar, gridvvar, lon, lats, substellar=substellarlon
        )
        ...
        if (ntru + 1) * (ntru + 2) in uvar.shape:  # spectral variable
            ...
            if len(uvar.shape) == 4:  # Include lev
                ...
                gpuvar = np.asfortranarray(
                    np.transpose(np.reshape(uvar, (ntimes * nlevs, nlat, nlon)))
                )
                gpvvar = np.asfortranarray(
                    np.transpose(np.reshape(vvar, (ntimes * nlevs, nlat, nlon)))
                )
                dims = ("time", levd, "modes", "complex")
            else:
                ...
                ntimes = uvar.shape[0]
                gpuvar = np.asfortranarray(
                    np.transpose(np.reshape(uvar, (ntimes, nlat, nlon)))
                )
                gpvvar = np.asfortranarray(
                    np.transpose(np.reshape(vvar, (ntimes, nlat, nlon)))
                )
                dims = ("time", "modes", "complex")
            spuvar, spvvar = pyfft.gpvsp(
                gpvuar, gpvvar, costhetadr, ntru, int(physfilter)
            )
        ...
        if (ntru + 1) * (ntru + 2) in uvar.shape:  # spectral variable
            ...
            if len(uvar.shape) == 4:  # include lev
                ...
                gpuvar = np.asfortranarray(
                    np.transpose(
                        np.reshape(
                            uvar / 1.4142135623730951, (ntimes * nlevs, nlat, nlon)
                        )
                    )
                )
                gpvvar = np.asfortranarray(
                    np.transpose(
                        np.reshape(
                            vvar / 1.4142135623730951, (ntimes * nlevs, nlat, nlon)
                        )
                    )
                )
                fcshape = (ntimes, nlevs, nlat, nlon // 2, 2)
                ...
                gpuvar = np.asfortranarray(
                    np.transpose(
                        np.reshape(uvar / 1.4142135623730951, (ntimes, nlat, nlon))
                    )
                )
                gpvvar = np.asfortranarray(
                    np.transpose(
                        np.reshape(vvar / 1.4142135623730951, (ntimes, nlat, nlon))
                    )
                )
                fcshape = (ntimes, nlat, nlon // 2, 2)
                dims = ["time", "lat", "fourier", "complex"]
        ...
        if len(rottlgriduvar.shape) == 4:  # include lev
            ...
            gpuvar = np.asfortranarray(
                np.transpose(
                    np.reshape(
                        rottlgriduvar / 1.4142135623730951, (ntimes * nlevs, nlat, nlon)
                    )
                )
            )
            gpvvar = np.asfortranarray(
                np.transpose(
                    np.reshape(
                        rottlgridvvar / 1.4142135623730951, (ntimes * nlevs, nlat, nlon)
                    )
                )
            )
            fcshape = (ntimes, nlevs, nlat, nlon // 2, 2)
            ...
            gpuvar = np.asfortranarray(
                np.transpose(
                    np.reshape(rottlgriduvar / 1.4142135623730951, (ntimes, nlat, nlon))
                )
            )
            gpvvar = np.asfortranarray(
                np.transpose(
                    np.reshape(rottlgridvvar / 1.4142135623730951, (ntimes, nlat, nlon))
                )
            )
            fcshape = (ntimes, nlat, nlon // 2, 2)
            dims = ["time", "lat", "fourier", "complex"]
...
def dataset(
    ...
    gridlnps, lnpsmeta = _transformvar(
        lon[:],
        lat[:],
        rawdata[str(lnpscode)][:],
        ilibrary[str(lnpscode)][:],
        nlat,
        nlon,
        nlev,
        ntru,
    ...
    for jlat in range(nlat):
        dpsdx[..., jlat, :] = np.gradient(
            gridlnps[..., jlat, :], rlon * plarad * colat[jlat], axis=-1
        )
    ...
    for key in variablecodes:
        ...
        if type(key) == int:
            ...
            if key in ilibrary:
                ...
                key = str(
                    kcode
                )  # Now key is always the integer code, and meta[0] is always the name
            else:
        ...
        if not derived:
            ...
            _log(
                logfile,
                "Collected variable: %8s\t.... %3d timestamps"
                % (meta[0], variable.shape[0]),
            )
        else:  # derived=True
            ...
            if key == str(ucode):  # ua
                if windless:
                    ...
                    ua, va, meta, vmeta = _transformvectorvar(
                        lon[:],
                        div,
                        vort,
                        umeta,
                ...
                if windless:
                    ...
                    vmeta = ilibrary[key][:]
                    vmeta.append(key)
                    ua, va, umeta, meta = _transformvectorvar(
                        lon[:],
                        div,
                        vort,
                        umeta,
                ...
                if windless:
                    ...
                    umeta = ilibrary[str(ucode)][:]
                    vmeta = ilibrary[str(vcode)][:]
                    ua, va, umeta, vmeta = _transformvectorvar(
                        lon[:],
                        div,
                        vort,
                        umeta,
                ...
                sruncode
            ):  # Precip + Evap - Increase in snow  = water added to bucket
                ...
                if not windless:
                    uu, vv, umeta, vmeta = _transformvectorvar(
                        lon[:],
                        div,
                        vort,
                        umeta[:],
                    ...
                    dv, dmeta = _transformvar(
                        lon[:],
                        lat[:],
                        div,
                        ilibrary[str(divcode)][:],
                        nlat,
                        nlon,
                        nlev,
                        ntru,
                ...
                if "ta" in rdataset:
                    ta, tameta = _transformvar(
                        lon[:],
                        lat[:],
                        rdataset["ta"][0][:],
                        ilibrary[str(tempcode)][:],
                        nlat,
                        nlon,
                        nlev,
                        ntru,
                ...
                if windless:
                    ...
                    ua, va, umeta2, vmeta2 = _transformvectorvar(
                        lon[:],
                        div,
                        vort,
                        umeta,
                ...
                sign = 1 - 2 * (
                    tempmode == "synchronous"
                )  # -1 for synchronous, 1 for equatorial
                ...
                slp[mask] = aph[mask] * np.exp(
                    ZPRT * (1.0 - ZPRTAL * (0.5 - ZPRTAL / 3.0))
                )
                ...
                if "hus" in rdataset:
                    qq, qmeta = _transformvar(
                        lon[:],
                        lat[:],
                        rdataset["hus"][0][:],
                        ilibrary[str(humcode)][:],
                        nlat,
                        nlon,
                        nlev,
                        ntru,
                ...
                gz[:, nlev, ...] = oro[
                    :
                ]  # bottom layer of geopotential Z is the orographic geopotential
                ...
                # water.
                zqsat = (
                    rdbrv * ra1 * np.exp(ra2 * (temp - TMELT) / (temp - ra4)) / pa
                )  # saturation spec hum
            ...
            elif key == str(thetahcode) or key == str(
                thetafcode
    ...
    rdataset["levp"] = [
        np.array(levp),
        ["levp", "half_sigma_coordinate", "nondimensional"],
    ]
...
def netcdf(rdataset, filename="most_output.nc", append=False, logfile=None):
    ...
    if append:
        ...
        levels = ncd.createVariable(
            "lev", "f4", ("lev",), zlib=True, least_significant_digit=6
        )
        hlevels = ncd.createVariable(
            "levp", "f4", ("levp",), zlib=True, least_significant_digit=6
        )
        ...
        fourierc = ncd.createVariable(
            "fharmonic", "f4", ("fourier",), zlib=True, least_significant_digit=6
        )
        spharmonics = ncd.createVariable(
            "modes", "f4", ("modes",), zlib=True, least_significant_digit=6
        )
    ...
    for key in keyvars:
        ...
        if "complex" in dims:  # Complex dtype
            ...
            if not append:
                ...
                try:
                    variable = ncd.createVariable(
                        key, complex64_t, dims, zlib=True, least_significant_digit=lsd
                    )
                except:
                    _log(logfile, meta)
                    raise
            ...
            if not append:
                ...
                try:
                    variable = ncd.createVariable(
                        key, "f4", dims, zlib=True, least_significant_digit=lsd
                    )
                except:
                    _log(logfile, meta)
                    raise
        ...
        _log(
            logfile,
            "Packing %8s in %s\t....... %d timestamps" % (key, filename, ntimes),
        )
...
def npsavez(rdataset, filename="most_output.npz", logfile=None):
    ...
    for key in keyvars:
        ...
        _log(
            logfile,
            "Packing %8s in %s\t....... %d timestamps"
            % (key, filename, rdataset[key][0].shape[0]),
        )
...
def _writecsvs(filename, variables, meta, extension=None, logfile=None):
    ...
    for var in dimvars:
        ...
        np.savetxt(
            ...
            header=(str(len(variables[var])) + ",|||," + ",".join(meta[var])),
            delimiter=",",
        )
        keyvars.remove(var)
    ...
    for var in keyvars:
        ...
        try:
            np.savetxt(
                outname,
                var2d.astype("float32"),
                header=(
                    ",".join(np.array(shape).astype(str))
                    + ",|||,"
                    + ",".join(meta[var][:-1])
                    + ",".join(meta[var][-1])
                ),
            ...
            print(meta[var])
            print(",".join(meta[var]))
            print(",".join(np.array(shape).astype(str)) + ",|||," + ",".join(meta[var]))
            raise
...
def csv(rdataset, filename="most_output.tar.gz", logfile=None, extracompression=False):
    ...
    if fileparts[-2] == "tar":  # We will be doing compression
        ...
        if extracompression:
            ext = ".gz"
        files, dirname = _writecsvs(
            filename, variables, meta, extension=ext, logfile=logfile
        )
            ...
            for var in files:
                ...
                try:
                    _log(
                        ...
                        % (var, filename, rdataset[var][0].shape[0]),
                    )
                except:
...
def hdf5(rdataset, filename="most_output.hdf5", append=False, logfile=None):
    ...
    if "lat" not in hdfile:
        hdfile.create_dataset(
            "lat",
            data=latitude[0].astype("float32"),
            compression="gzip",
            compression_opts=9,
            shuffle=True,
            fletcher32=True,
    ...
    if "lon" not in hdfile:
        hdfile.create_dataset(
            "lon",
            data=longitude[0].astype("float32"),
            compression="gzip",
            compression_opts=9,
            shuffle=True,
            fletcher32=True,
    ...
    for var in keyvars:
        if var not in hdfile:
            ...
            hdfile.create_dataset(
                var,
                data=rdataset[var][0].astype("float32"),
                compression="gzip",
                maxshape=maxshape,
                compression_opts=9,
                shuffle=True,
            ...
            hdfile[var].resize(
                (hdfile[var].shape[0] + rdataset[var][0].shape[0]), axis=0
            )  # Expand file
            hdfile[var][-rdataset[var][0].shape[0] :] = rdataset[var][0].astype(
                "float32"
            )  # Append
        _log(
            logfile,
...
def postprocess(
        ...
        for line in fnamelist:
            ...
            if len(parts) > 1:
                ...
                if key == "code" or key == "CODE":
                    ...
                    radius = (
                        float(arg) / 6371220.0
                    )  # Radii in the namelist are given in metres
                elif key == "gravity" or key == "GRAVITY":
                    gravity = float(arg)
                elif (key == "mars" or key == "MARS") and int(arg) == 1:
                    gravity = MARS_GRAV
                    radius = (
                        MARS_RADIUS / 6371220.0
                    )  # We want to start off with radii in Earth radii
    ...
    if type(times) == int:  # A number of time outputs was specified
        ...
        if ntimes == times:  # The number of outputs exactly equals the number provided
            timeaverage = (
                False  # This way stdev still gets computed, but over the whole file.
            )
            ...
            if timeaverage:
                _log(
                    logfile,
                    "\nComputing averages, going from %d timestamps to %d ..."
                    % (ntimes, times),
                )
                if times > ntimes and interpolatetimes:
                    ...
                    indices = (
                        np.digitize(
                            np.linspace(dtimes[0], dtimes[-1], num=times + 1), ttimes
                        )
                        - 1
                    )
                    ...
                    for var in varkeys:
                        ...
                        interpfunc = scipy.interpolate.interp1d(
                            dtimes, odata, axis=0, kind="linear"
                        )
                    ...
                    for var in varkeys:
                        ...
                        newcounts = np.transpose(np.resize(counts, newshape[::-1]))
                        data[var][0] = (
                            np.add.reduceat(odata, indices[:-1], axis=0) / newcounts
                        )
                        if stdev:  # Compute standard deviation
                            stdvar = np.zeros(data[var][0].shape)
                            for nt in range(stdvar.shape[0]):
                                stdvar[nt, ...] = np.std(
                                    odata[indices[nt] : indices[nt + 1]], axis=0
                                )
                            stdmeta = list(data[var][1][:])
                            stdmeta[0] += "_std"
                            stdmeta[1] += "_standard_deviation"
                ...
                if interpolatetimes:
                    ...
                    _log(
                        logfile,
                        "\nInterpolating from %d timestamps to %d ..."
                        % (ntimes, times),
                    )
                else:
                ...
                newtimes = np.linspace(
                    dtimes[0], dtimes[-1], num=times
                )  # We can't extrapolate; only interpolate
                varkeys = list(data.keys())
                varkeys.remove("time")
                varkeys.remove("lat")
                varkeys.remove("lon")
                ...
                for var in varkeys:
                    odata = data[var][0][:]
                    interpfunc = scipy.interpolate.interp1d(
                        dtimes, odata, axis=0, kind=interpolation
                    )
                    data[var][0] = interpfunc(newtimes)
                if not interpolatetimes:
                    interpfunc = scipy.interpolate.interp1d(
                        dtimes, dtimes, kind="nearest"
                    )
        ...
        if timeaverage:  # times values are assumed to be bin edges
            ...
            if (
                not interpolatetimes
            ):  # We will always round down to the nearest neighbor
                _log(
                    logfile,
                ...
                indices = (
                    np.digitize(
                        np.array(times) * (dtimes[-1] - dtimes[0]) + dtimes[0], dtimes
                    )
                    - 1
                )
                counts = np.diff(indices)
                ...
                indices = (
                    np.digitize(
                        np.array(times) * (dtimes[-1] - dtimes[0]) + dtimes[0], ttimes
                    )
                    - 1
                )
                counts = np.diff(indices)
            ...
            newtimes = (
                np.array(times) * (dtimes[-1] - dtimes[0]) + dtimes[0]
