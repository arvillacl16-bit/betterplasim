...
def basicclouds(pressure, temperature, cloudwater):
    ...
    radius["H2O(c)"][temperature <= 216.21] = (
        975.51 * np.exp(0.05 * (temperature[temperature <= 216.21] - 279.5)) + 0.66
    ) * 1e-4
    radius["H2O(c)"][temperature > 216.21] = (
        175.44 * np.exp(0.05 * (temperature[temperature > 216.21] - 279.5)) + 34.45
    ) * 1e-4
...
def _adistance(lons, lats, tlon, tlat):
    ...
    rtlat = tlat * np.pi / 180.0
    rtlon = tlon * np.pi / 180.0
    rlons = lons * np.pi / 180.0
    rlats = lats * np.pi / 180.0
    distance = abs(
        np.arccos(
            np.sin(rtlat) * np.sin(rlats)
            + np.cos(rtlat) * np.cos(rlats) * np.cos(rlons - rtlon)
        )
    )
    return distance
...
def _smooth(array, xcoords=None, xN=None, centerweight=0.95):
    ...
    if xcoords is None:
        newarr = (
            edgeweight * padded[:-2]
            + centerweight * padded[1:-1]
            + edgeweight * padded[2:]
        )
    else:
        dx = np.diff(
            np.append(
                np.append(0.5 * xcoords[0], 0.5 * (xcoords[:-1] + xcoords[1:])), xN
            )
        )
...
def _airmass(pa, ps, ta, hus, gascon, gravity):
    ...
    airdensity = pa * 100.0 / (gascon * vtemp)
                ...
                for j in range(k, -1, -1):
                    if newhum[j] > minvalue:
                        if (newhum[j] - minvalue) * airmass[j] >= excessmass[k]:
                            newhum[j] = (
                                newhum[j] * airmass[j] - excessmass[k]
                            ) / airmass[j]
                            excessmass[k] = 0.0
                            break
...
def _transitcolumn(
    ...
    for k in range(len(extp) - 1, 0, -1):
        ...
        zo3 = -(ozone + ozone * zconst) / (1.0 + np.exp((zh - bo3) / co3)) + zo3t
        mass_fractions["O3"][k] = zo3 * ga / (zfo3 * extdp[k])
        zo3t -= zo3
    mass_fractions["O3"][0] = zo3t * ga / (zfo3 * extdp[0])

    if smooth:
        mass_fractions["O3"] = _smooth(
            mass_fractions["O3"], xcoords=extp, xN=psurf, centerweight=smoothweight
        )
...
def transit(
    ...
    atmosphere = Radtrans(
        line_species=["H2O_" + h2o_lines, "CO2", "O3"],
        rayleigh_species=["N2", "O2", "CO2", "H2", "He"],
        continuum_opacities=["N2-N2", "N2-O2", "O2-O2", "CO2-CO2", "H2-H2", "H2-He"],
    ...
    for idx, t in enumerate(transittimes):
        _log(
            logfile,
            "Configuring columns for timestamp %d corresponding to timestep %d....."
            % (t, output.variables["time"][t]),
        )
        ...
        terminator_mask = darkness * (
            np.sqrt(
                np.gradient(darkness, axis=0) ** 2 + np.gradient(darkness, axis=1) ** 2
            )
            > 0.0
        )
        ...
        rlats = tlats[:, 0] * np.pi / 180.0
        o3 = (
            a0o3
            + a1o3 * abs(np.sin(rlats))
            + aco3 * np.sin(rlats) * np.cos(2 * np.pi * (dt - toffo3))
        )
        ...
        _log(
            logfile,
            "Terminator mask applied; there are %d columns along the terminator."
            % nterm,
        )
        ...
        if num_cpus > 1:
            ...
            _log(
                logfile,
                "%d processes will be spun up; if this uses a substantial fraction\n"
                % num_cpus
        ...
        _log(
            logfile,
            "All columns computed! Mean transit spectrum is now being computed.",
        )
        _log(logfile, "\n")
    ...
    nptransits = (
        np.zeros((len(transittimes), maxlen, len(atmosphere.freq))) - 9999999.0
    )  # finite fill value
...
def _imgcolumn(
    ...
    if (
        zenith <= 90.0
    ):  # We only need to compute the direct light contribution if the sun is up
    ...
    atmosphere.calc_flux(
        extta, mass_fractions, gravity, MMW, geometry="non-isotropic", **kwargs
    )
    ...
    return (
        flux,
        intensities,
        atmosphere.stellar_intensity
        * np.maximum(0.0, np.cos(zenith * np.pi / 180.0))
        * np.pi
        * 1e6,
    )  # erg cm-2 s-1 Hz-1
...
def orennayarcorrection_col(
    intensity, lon, lat, sollon, sollat, zenith, observer, albedo, sigma
):
    ...
    rlatz = sollat * np.pi / 180.0
    rlonz = sollon * np.pi / 180.0
    rlons = lon * np.pi / 180.0
    rlats = lat * np.pi / 180.0
    phi = np.arctan2(
        -np.cos(rlatz) * np.sin(rlonz - rlons),
        -(
            np.sin(rlatz) * np.cos(rlats)
            - np.cos(rlatz) * np.sin(rlats) * np.cos(rlonz - rlons)
        ),
    )
    if phi < 0:
        phi += 2 * np.pi

    rlono = observer[1] * np.pi / 180.0
    rlato = observer[0] * np.pi / 180.0
    otheta = np.arccos(
        np.sin(rlats) * np.sin(rlato)
        + np.cos(rlats) * np.cos(rlato) * np.cos(rlono - rlons)
    )
    if otheta > np.pi / 2.0:
        return 0.0
    ophi = np.arctan2(
        -np.cos(rlato) * np.sin(rlono - rlons),
        -(
            np.sin(rlato) * np.cos(rlats)
            - np.cos(rlato) * np.sin(rlats) * np.cos(rlono - rlons)
        ),
    )
    if ophi < 0:
        ophi += 2 * np.pi
    ...
    c2 = (
        0.45
        * (sigma**2 / (sigma**2 + 0.05))
        * (np.sin(a) - (2 * b / np.pi) ** 2 * (np.cos(dphi) < 0))
    )
    c3 = 0.125 * (sigma**2 / (sigma**2 + 0.09)) * (4 * a * b / np.pi**2) ** 2

    L1coeff = (
        c1
        + np.cos(dphi) * min(10.0, np.tan(b)) * c2
        + (1 - abs(np.cos(dphi)) * min(10.0, np.tan((a + b) / 2.0))) * c3
    )
    L2coeff = albedo * (
        0.17
        * (sigma**2 / (sigma**2 + 0.13))
        * (1 - np.cos(dphi) * (2 * b / np.pi) ** 2)
    )
    L = L1coeff + L2coeff
...
def makecolors(intensities, gamma=True, colorspace="sRGB"):
    ...
    for k in range(flatshape):
        colors[k, :] = cmatch.xyz2rgb(
            flatintensities[k, 0], flatintensities[k, 1], norms[k], gamut=colorspace
        )
    colors /= np.nanmax(colors)
    colors = np.reshape(colors, ogshape)
    if gamma is True:
        colors[colors < 0.0031308] = 12.92 * colors[colors < 0.0031308]
        colors[colors >= 0.0031308] = (
            1.055 * colors[colors >= 0.0031308] ** (1.0 / 2.4) - 0.055
        )
    elif gamma is not None:
        colors = colors ** (1.0 / gamma)
    return colors
...
def image(
    ...
    for jlat in range(0, nlat):
        for jlon in range(0, nlon):
            ...
            darea[jlat, jlon] = abs(np.sin(lt1[jlat]) - np.sin(lt1[jlat + 1])) * abs(
                dln
            )
    darea = darea.flatten()

    surfaces = [
        spec.modelspecs["groundblend"] * 0.01,
    ...
    if "veglai" in output.variables:
        sfcalbedo = (
            surfaces[1][np.newaxis, :] * sea[:, np.newaxis]
            + surfaces[5][np.newaxis, :] * (1 - sea)[:, np.newaxis]
        )
    else:
        sfcalbedo = (
            surfaces[1][np.newaxis, :] * sea[:, np.newaxis]
            + surfaces[0][np.newaxis, :] * (1 - sea)[:, np.newaxis]
        )
    ...
    if orennayar and sigma is None:
        sigmas = [
            ...
            0.9,
            0.95,
        ]  # roughnesses for ground,ocean,ice/snow,and clouds
    ...
    for idx, t in enumerate(imagetimes):
        ...
        dql = np.ma.getdata(
            np.concatenate([dql, np.zeros(len(dql))[:, np.newaxis]], axis=1)
        )
        ...
        zenith = np.arccos(czen) * 180.0 / np.pi
        ...
        zenith[nightside <= 0.5] = np.minimum(
            zenith[nightside <= 0.5], 90.0 - 360.0 / nlon * 0.5
        )  # dawn/dusk angle due to finite resolution
        ...
        if allforest:
            ...
            if (
                "veglai" in output.variables
            ):  # Use dune sand for deserts and bare rock for mountaintops
                vlai = output.variables["veglai"][t, ...]
                forest = (
                    np.exp(-vlai) * vlai
                    + (1.0 - np.exp(-vlai)) * (1.0 - np.exp(-0.5 * vlai)) ** vegpowerlaw
                ).flatten()  # Fraction of PAR that is absorbed by vegetation
                desertf = (
                    (output.variables["lsm"][t, ...] > 0.5)
                    * 1.0
                    * (output.variables["vegsoilc"][t, ...] < 0.01)
                    * (output.variables["mrso"][t, ...] < 0.01)
                ).flatten()
                mntf = (
                    1.0 * (output.variables["netz"][t, ...] > baremountainz).flatten()
                )
            else:
        ...
        albedo[forest > 0] = (1 - forest[forest > 0]) * albedo[forest > 0] + forest[
            forest > 0
        ] * 0.3
        bare = 1 - (ice + forest)
        ...
        surfspecs = (
            surfspecs * (1 - output.variables["sic"][t, ...].flatten())[:, np.newaxis]
            + surfaces[2][np.newaxis, :]
            * (output.variables["sic"][t, ...].flatten())[:, np.newaxis]
        )
        surfspecs = (
            surfspecs * bare[:, np.newaxis]
            + surfaces[2][np.newaxis, :] * ice[:, np.newaxis]
            + surfaces[3][np.newaxis, :] * forest[:, np.newaxis]
        )

        clt = output.variables["clt"][t, ...].flatten()
        if orennayar and sigma is None:
            surfsigmas = sfcsigmas * (
                1 - output.variables["sic"][t, ...].flatten()
        ...
        if consistency:
            for n in range(len(albedo)):
                bol_alb = np.trapz(
                    stellarspec * surfspecs[n, :], x=spec.wvl
        ...
        for idv in range(projectedareas.shape[1]):
            ...
            projectedareas[idx, idv, :][view < np.pi / 2] = (
                np.cos(view[view < np.pi / 2]) * darea[view < np.pi / 2]
            )
        ...
        if num_cpus > 1:
            args = zip(
                ...
                repeat(gravity),
                repeat(Tstar),
                repeat(Rstar * nc.r_sun),
                repeat(starseparation * nc.AU),
                zenith,
                repeat(cloudfunc),
                repeat(smooth),
                repeat(smoothweight),
            ...
            for k in range(broadrefl.shape[0]):
                ...
                try:
                    broadrefl[k] = np.trapz(convolved[k], x=viswvl, axis=1) / np.trapz(
                        influx, x=viswvl, axis=1
                    )
                except:
            ...
            if orennayar and sigma.max() > 0.0:
                for idv in range(observers.shape[0]):
                    args = zip(
                        photos[idx, 0, :, 2].flatten(),
                        ilons,
                        ilats,
                        repeat(sollon),
                        repeat(sollat),
                        zenith,
                        repeat(observers[idv, :]),
                        broadrefl,
                        sigma,
                    )
                    with mp.Pool(num_cpus) as pool:
            ...
            if orennayar and sigma.max() > 0.0:
                for idv in range(observers.shape[0]):
                    photos[idx, idv + 1, :, 2] = orennayarcorrection(
                        photos[idx, 0, :, 2],
                        ilons,
                        ilats,
                        sollon,
        ...
        for idv in range(photos.shape[1]):
            photos[idx, idv, ...] = makecolors(
                photos[idx, idv, ...], gamma=gamma, colorspace=colorspace
            )
        ...
        try:
            for idv in range(projectedareas.shape[1]):
                ...
                meanimages[idx, idv, :] = np.ma.average(
                    np.ma.MaskedArray(
                        images[idx, ...], mask=np.isnan(images[idx, ...])
                    ),
                    axis=0,
                    weights=view,
                )
    ...
    ice = np.minimum(ice / 0.02, 1.0)  # 0-1 with a cap at 2 cm of snow
    snow = 1.0 * (
        output.variables["snd"][0, ...].flatten() > 0.02
    ...
    surfspecs = (
        sfcalbedo * (1 - output.variables["sic"][0, ...].flatten())[:, np.newaxis]
        + surfaces[2][np.newaxis, :]
        * (output.variables["sic"][0, ...].flatten())[:, np.newaxis]
    )
    surfspecs = (
        surfspecs * bare[:, np.newaxis]
        + surfaces[2][np.newaxis, :] * ice[:, np.newaxis]
    ...
    for n in range(len(albedo)):
        bol_alb = np.trapz(stellarspec * surfspecs[n, :], x=spec.wvl) / np.trapz(
            stellarspec, x=spec.wvl
        )
        fudge_factor = albedo[n] / bol_alb
        surfspecs[n, :] *= fudge_factor
    ...
    photos = np.reshape(
        photos, (len(imagetimes), obsv_coords.shape[1] + 1, nlat, nlon, 3)
    )
    if debug:
        projectedareas = np.reshape(
            projectedareas, (len(imagetimes), obsv_coords.shape[1], nlat, nlon)
        )
        albedomap = np.reshape(
            albedomap, (len(imagetimes), nlat, nlon, len(surfaces[0]))
        )
...
_postmetadata = {
    "images": ["image_spectra_map", "erg cm-2 s-1 Hz-1"],
    "spectra": [
        ["image_spectrum_avg", "erg cm-2 s-1 Hz-1"],
...
def _writecsvs(filename, variables, extension=None, logfile=None):
    ...
    os.system(
        "mkdir %s" % dirname
    )  # Create a subdirectory that just omits the file extension
    files = []
    ...
    for var in dimvars:
        ...
        np.savetxt(
            outname,
            variables[var].astype("float32"),
            header=(str(len(variables[var])) + ",|||," + ",".join(_postmetadata[var])),
            delimiter=",",
        )
        keyvars.remove(var)
    ...
    for var in keyvars:
        ...
        var2d = np.reshape(
            variables[var], (dim1, dim2)
        )  # np.savetxt can only handle 2 dimensions
        outname = "%s/%s_%s%s" % (dirname, fname, var, extension)
        if var == "spectra":
        ...
        try:
            np.savetxt(
                outname,
                var2d.astype("float64"),
                header=(
                    ",".join(np.array(shape).astype(str)) + ",|||," + ",".join(meta)
                ),
                delimiter=",",
            )
        except:
            ...
            print(meta)
            print(",".join(meta))
            print(",".join(np.array(shape).astype(str)) + ",|||," + ",".join(meta))
            raise
...
def _csv(dataset, filename="most_output.tar.gz", logfile=None, extracompression=False):
    ...
    if fileparts[-2] == "tar":  # We will be doing compression
        ...
        with tarfile.open(filename, "w:%s" % fileparts[-1]) as tarball:
            ...
            for var in files:
                ...
                try:
                    _log(
                        logfile,
                        (writeline + " ....... %d timestamps")
                        % (var, filename, dataset[var].shape[0]),
                    )
                except:
        ...
        fileparts[-1] == "tar"
    ):  # We're still making a tarball, but it won't be compressed
        import tarfile
...
def _netcdf(filename, dataset, logfile=None):
    ...
    times = ncd.createVariable(
        "time", "f4", ("time",), zlib=True, least_significant_digit=6
    )
    wvl = ncd.createDimension("wvl", nwvls)
    wavelengths = ncd.createVariable(
        "wvl", "f4", ("wvl",), zlib=True, least_significant_digit=6
    )
    ...
    if twoD:
        ...
        latitudes = ncd.createVariable(
            "lat", "f4", ("lat",), zlib=True, least_significant_digit=6
        )
        longitudes = ncd.createVariable(
            "lon", "f4", ("lon",), zlib=True, least_significant_digit=6
        )
        ...
        observersp = ncd.createVariable(
            "observersp", "i4", ("observer_plus",), zlib=True
        )
        ...
        rgb = ncd.createVariable(
            "rgb", "f4", ("RGB",), zlib=True, least_significant_digit=6
        )
        ...
        latitudes = ncd.createVariable(
            "lat", "f4", ("time", "coord"), zlib=True, least_significant_digit=6
        )
        longitudes = ncd.createVariable(
            "lon", "f4", ("time", "coord"), zlib=True, least_significant_digit=6
        )
    ...
    if twoD:
        ...
        if len(dvarmask) == 0:
            ...
            lsd = max(
                int(round(abs(np.log10(abs(dvarmask).min())) + 0.5)) + 6, 6
            )  # get decimal place of smallest value
            if abs(dvarmask).min() >= 1.0:
                lsd = 6
        images = ncd.createVariable(
            "images",
            "f8",
            ("time", "lat", "lon", "wvl"),
            zlib=True,
        ...
        dvarmask = datavar[np.isfinite(datavar)]
        dvarmask = dvarmask[dvarmask != 0]
        photos = ncd.createVariable(
            "colors",
            "f4",
            ("time", "observersp", "lat", "lon", "RGB"),
            zlib=True,
        ...
        _log(
            logfile,
            "Writing a spectrum for each terminator column for each observation.....",
        )
        ...
        weights = ncd.createVariable(
            "weights", "f4", ("time", "coord"), zlib=True, least_significant_digit=6
        )
        ...
        if len(dvarmask) == 0:
            ...
            if abs(dvarmask).min() >= 1.0:
                lsd = 6
        spectra = ncd.createVariable(
            "spectra", "f8", ("time", "wvl"), zlib=True, least_significant_digit=lsd
        )
        spectra.set_auto_mask(False)
        spectra[:] = datavar[:]
        spectra.units = "km"
...
def _hdf5(filename, dataset, logfile=None, append=False):
    ...
    if "lat" not in hdfile:
        if "images" in dataset:
            ...
            hdfile.create_dataset(
                ...
                data=latitude[:].astype("float32"),
                compression="gzip",
                maxshape=[None, dataset["lat"].shape[1]],
                compression_opts=9,
                shuffle=True,
                fletcher32=True,
            )
        hdfile.attrs["lat"] = np.array(
            ["latitude".encode("utf-8"), "degrees".encode("utf-8")], dtype=u8t
        )
    ...
    if "lon" not in hdfile:
        if "images" in dataset:
            ...
            hdfile.create_dataset(
                ...
                data=longitude[:].astype("float32"),
                compression="gzip",
                maxshape=[None, dataset["lon"].shape[1]],
                compression_opts=9,
                shuffle=True,
                fletcher32=True,
            )
        hdfile.attrs["lon"] = np.array(
            ["longitude".encode("utf-8"), "degrees".encode("utf-8")], dtype=u8t
        )
    elif "transits" in hdfile:
    ...
    if "wvl" not in hdfile:
        hdfile.create_dataset(
            "wvl",
            data=wvls[:].astype("float32"),
            compression="gzip",
            compression_opts=9,
            shuffle=True,
            fletcher32=True,
        )
        hdfile.attrs["wvl"] = np.array(
            ["wavelength".encode("utf-8"), "microns".encode("utf-8")], dtype=u8t
        )
    if "time" not in hdfile:
        ...
        hdfile.attrs["time"] = np.array(
            ["obsv_time_index".encode("utf-8"), "indices".encode("utf-8")], dtype=u8t
        )
    else:
    ...
    for var in keyvars:
        if var not in hdfile:
            ...
            hdfile.create_dataset(
                var,
                data=dataset[var].astype("float64"),
                compression="gzip",
                maxshape=maxshape,
                compression_opts=9,
                shuffle=True,
            ...
            if "images" in keyvars and var == "spectra":
                hdfile.attrs[var] = np.array(
                    [
                        "image_spectrum_avg".encode("utf-8"),
                        "erg cm-2 s-1 Hz-1".encode("utf-8"),
                    ],
                    dtype=u8t,
                )
            elif "transits" in keyvars and var == "spectra":
                hdfile.attrs[var] = np.array(
                    ["transit_spectrum_avg".encode("utf-8"), "km".encode("utf-8")],
                    dtype=u8t,
                )
            elif var == "images":
                hdfile.attrs[var] = np.array(
                    [
                        "image_spectra_map".encode("utf-8"),
                        "erg cm-2 s-1 Hz-1".encode("utf-8"),
                    ],
                    dtype=u8t,
                )
                ...
                hdfile.attrs[var] = np.array(
                    ["true_color_image".encode("utf-8"), "RGB".encode("utf-8")],
                    dtype=u8t,
                )
            elif var == "albedomap":
                hdfile.attrs[var] = np.array(
                    [
                        "observed_reflectivity".encode("utf-8"),
                        "dimensionless".encode("utf-8"),
                    ],
                    dtype=u8t,
                )
            elif var == "transits":
                hdfile.attrs[var] = np.array(
                    ["transit_spectra_map".encode("utf-8"), "km".encode("utf-8")],
                    dtype=u8t,
                )
            elif var == "weights":
                hdfile.attrs[var] = np.array(
                    [
                        "column_contribution_weights".encode("utf-8"),
        ...
        _log(
            logfile,
            "Packed %21s in %s\t...... %d timestamps"
            % (hdfile.attrs[var][0], filename, dataset[var].shape[0]),
        )
...
def save(filename, dataset, logfile=None, extracompression=False):
    ...
    if fileparts[-1] == "nc":
        ...
        fileparts[-2] + "." + fileparts[-1]
    ) in ("tar.gz", "tar.bz2", "tar.xz"):
        output = _csv(
            filename, dataset, logfile=logfile, extracompression=extracompression
        )
        return output
