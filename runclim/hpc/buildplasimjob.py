...
def prep(job):
    ...
    for name in job.fields[2:]:
        ...
        if name == "pN2":
            ...
            if "pCO2" in fields:
                ...
                edit_namelist(
                    ...
                    "radmod_namelist",
                    "CO2",
                    str(float(job.parameters["pCO2"]) / p0 * 1.0e6),
                )
        ...
        if name == "radius":
            found = True
            edit_namelist(
                home, "planet_namelist", "PLARAD", str(float(val) * 6371220.0)
            )
            postnls = glob.glob(home + "/*.nl")
            for postf in postnls:
                pf = postf.split("/")[-1]
        ...
        if name == "rcrit":
            ...
            exec("critlevs=" + val)
            edit_namelist(
                home, "rainmod_namelist", "RCRIT", ",".join(critlevs.astype(str))
            )
        ...
        if name == "timestep":
            ...
            edit_namelist(
                home, "plasim_namelist", "NSTPS", str(21600 / int(float(val)))
            )
            nsn = True
    ...
    jobscript += (
        "rsync -avzhur stuff.tar.gz /mnt/node_scratch/"
        + USER
        + "/"
        + home
        + "/         \n"
