...
def prep(job):
    ...
    print(
        "Setting stuff for job "
        + sig
        + " in plasim/job"
        + jid
        + " which is task number "
        + pid
    )
    ...
    for name in job.fields[2:]:
        ...
        if (
            name == "lockedyear"
        ):  # Year length in days for a tidally-locked planet_namelist, and lon0.
            found = True
            val0 = val.split("/")[0]
            val1 = val.split("/")[1]
        ...
        if name == "drycore":
            ...
            if int(val) == 1:
                ...
                edit_namelist(
                    jid, "fluxmod_namelist", "NEVAP", "0"
                )  # Turn off evaporation

        if (
            name == "soilheat"
        ):  # 0.001 J/m^3/K = 1 J/kg/K. Ocean = 4180 J/kg/K = 4.18e6 J/m^3/K
            found = True
        ...
        if name == "rcrit":
            ...
            exec("critlevs=" + val)
            edit_namelist(
                jid, "rainmod_namelist", "RCRIT", ",".join(critlevs.astype(str))
            )
        ...
        if not found:  # Catchall for options we didn't include
            ...
            if len(args) > 1:
                ...
                print(
                    "Unknown parameter "
                    + name
                    + "! Submit unsupported parameters as KEY@NAMELIST in the header!"
                )
    ...
    jobscript += (
        "rsync -avzhur stuff.tar.gz /mnt/node_scratch/paradise/job"
        + jid
        + "/         \n"
        + "rm -rf stuff.tar.gz                     \n"
        + "cd /mnt/node_scratch/paradise/job"
        + jid
        + "/              \n"
    ...
    if keeprs:
        jobscript += (
            "cp plasim_restart ../output/" + job.name + "_restart            \n"
        )
    if monitor:
        jobscript += "python monitor_balance.py                                   \n"

    jobscript += (
        "   [ -e balance.log ] && cp balance.log ../output/"
        + job.name
        + "_balance.log    \n"
        + "   [ -e slopes.log ] && cp slopes.log ../output/"
        + job.name
        + "_slopes.log    \n"
        + "   python synthoutput.py MOST 1                                      \n"
        + "   cp MOST_history.npy ../output/"
        + job.name
        + "_history.npy            \n"
        + "   python "
        + cleanup
...
def submit(job):
    os.system(
        "cd plasim/job" + str(job.home) + " && " + SUB + " runplasim && cd " + job.top
    )
    time.sleep(1.0)
    tag = job.getID()
    job.write()
