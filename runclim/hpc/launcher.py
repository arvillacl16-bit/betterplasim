...
if __name__ == "__main__":
    ...
    while (
        openjobs
    ):  # We are using less than our full allocation, and the priority list is empty.
            ...
            for i in range(mark, -1, -1):  # Find header for the task
                if tasks[i][0] == "#":
                    ...
                    if (len(header.split()) - 1) == (
                        len(task)
                    ):  # first header with the right number of args
                        break
                    else:
                        header = False
        ...
        if ready:
            ...
            newjob = Job(
                header, " ".join(task), -1
            )  # Collect and organize the job parameters
