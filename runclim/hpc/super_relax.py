...
def energybalanced(threshhold=1.0e-4, baseline=50):  # Takes an average of 200 years
    ...
    if len(files) < baseline:  # Run for minimum of baseline years
        ...
        if (
            savgslope < threshhold and tavgslope < threshhold
...
if __name__ == "__main__":
    if gplasim:
        ...
        wf.write(
            "     CO2       AVG SURF T   WEATHERING    OUTGASSING      DpCO2       NEW CO2\n"
        )
        wf.close()
    EXP = "MOST"
    ...
    relaxed = False
    while (
        (year < minyears or not energybalanced(threshhold=4.0e-4))
        and year < maxyears
        and (time.clock() - tstart) <= TIMELIMIT
    ):
        year += 1
        ...
        os.system(
            "[ -e " + dataname + " ] && cp " + dataname + " " + EXP + "_OUT.%04d" % year
        )
