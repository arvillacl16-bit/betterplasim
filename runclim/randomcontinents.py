...
def writePGM(name, heightfield):
    ...
    img = ((np.round((heightfield / heightfield.max()) * 65535)).astype(int)).astype(
        str
    )
...
def generate(
    ...
    rcoord = 2 * np.arcsin(
        np.sqrt(
            np.sin(0.5 * rlats) ** 2 + np.cos(rlats) * np.sin(0.5 * (rlons - rl0)) ** 2
        )
    )
    ...
    while landarea <= landfraction - np.nanmin(darea):
        while True:
            ...
            if (
                grid[ilt, iln] < 0.5
                and np.sum(wgrid[ilt : ilt + 3, iln : iln + 3]) > 0.5
            ):
                goodtogo = True
                if (
                    np.sum(wgrid[ilt : ilt + 3, iln : iln + 3]) < 5.0
                ):  # Try to bias towards land-locked ocean cells
    ...
    if not orthographic and plot:
        ...
        if plot:
            ...
            ax.set_theta_zero_location("N")
            ax.pcolormesh(
                x, np.sin(y), z, cmap="gist_ncar", vmin=-0.3, vmax=ncontinents + 2
            )
            ax.set_rticks([])
            ax.set_thetagrids([])
            plt.title("Continental Cratons")
    ...
    if ntopo:
        ...
        if not orthographic and plot:
            ...
            if plot:
                fig, ax = plt.subplots(
                    subplot_kw={"projection": "polar"}, figsize=(9, 9)
                )
                ax.set_theta_zero_location("N")
        ...
        geozspline = interpolate.RectBivariateSpline(
            lns,
            lts[::-1],
            np.transpose(geo[::-1, :]),
            bbox=[lnsz[0], lnsz[-1], ltsz[-1], ltsz[0]],
            kx=kx,
            ky=ky,
        )
        geoz = np.transpose(geozspline(lnsz, ltsz[::-1]))
        geoz = geoz[::-1, :]
        contzspline = interpolate.RectBivariateSpline(
            lns,
            lts[::-1],
            np.transpose(grid[::-1, :]),
            bbox=[lnsz[0], lnsz[-1], ltsz[-1], ltsz[0]],
        ...
        topo = (
            np.copy(geoz) + contz * g0 * 10.0
        )  # Default lowlands of 10 meters above sea level
        ...
        for i in range(0, maxiters):
            ...
            for jlat in range(0, NLATZ):
                ...
                for jlon in range(0, NLONZ):
                    ...
                    nutop[jlat, jlon] = (
                        0.2 * topo[jlat, jlon]
                        + np.nanmean([c1, c2, c3, c4, c5, c6, c7, c8]) * 0.8
                    )
            topo[:] = nutop[:]
        topospline = interpolate.interp2d(
            lnsz, ltsz[::-1], (topo[::-1, :]), kind="linear"
        )
        ...
        if not orthographic and plot:
            plt.close("all")
            t = plt.pcolormesh(
                hlons, hlats, dtopo, cmap="gist_earth", norm=colors.LogNorm(vmin=10.0)
            )
            plt.xlabel("Degrees Longitude")
            plt.ylabel("Degrees Latitude")
            plt.ylim(np.amin(lts), np.amax(lts))
            ...
            if plot:
                ...
                ax.set_theta_zero_location("N")
                t = ax.pcolormesh(
                    x, np.sin(y), z, cmap="gist_earth", norm=colors.LogNorm(vmin=10.0)
                )
                ax.set_rticks([])
                ax.set_thetagrids([])
                plt.title("Topography")
...
def main():
    ...
    **Options**
    ...
    parser = ag.ArgumentParser(
        description="Randomly generate continents up to a specified land-fraction. Topography optional."
    )
    parser.add_argument(
        "-z",
        "--topo",
        action="store_true",
        help="Generate topographical geopotential map",
    )
    parser.add_argument(
        "-c", "--continents", type=int, default=7, help="Number of continental cratons"
    )
    parser.add_argument(
        "-f", "--landfraction", type=float, default=0.29, help="Land fraction"
    )
    parser.add_argument(
        "-n", "--name", default="Alderaan", help="Assign a name for the planet"
    )
    parser.add_argument(
        "-m",
        "--maxz",
        default=10.0,
        type=float,
        help="Maximum elevation in km assuming Earth gravity",
    )
    if os.path.exists("T21.nc"):
        parser.add_argument(
            "--nlats",
            type=int,
            help="Number of latitudes (evenly-spaced)--will also set longitudes (twice as many). If unset, PlaSim latitudes and longitudes will be used (T21 resolution; requires netCDF4)",
        )
    else:
        parser.add_argument(
            "--nlats",
            default=32,
            type=int,
            help="Number of latitudes (evenly-spaced)--will also set longitudes (twice as many).",
        )
    parser.add_argument(
        "-l",
        "--hemispherelongitude",
        type=float,
        default=np.nan,
        help="Confine land to a hemisphere centered on a given longitude",
    )
    parser.add_argument(
        "-o",
        "--orthographic",
        action="store_true",
        help="Plot orthographic projections centered on hemispherelongitude",
    )
    parser.add_argument(
        "-p",
        "--plot",
        action="store_true",
        help="Display plots of the generated continents",
    )
    ...
    output = generate(
        ...
        maxz=args.maxz,
        nlats=args.nlats,
        hemispherelongitude=args.hemispherelongitude,
        topo=args.topo,
...
if __name__ == "__main__" and (
    Path(sys.argv[0]).name != "sphinx-build" and Path(sys.argv[0]).name != "build.py"
):
    main()
